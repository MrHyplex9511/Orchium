...
The orchium server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@orchium/llm` tool loop after replacing its
...
- plugins get global "orchium" instance like in that post i showed
- orchium instance has stuff like `orchium.session.prompt()` or
  `orchium.tool.register({...})`
...
- decide whether to preserve deprecated `@orchium/llm` orchestration exports
