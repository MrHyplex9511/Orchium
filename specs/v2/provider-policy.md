...
Both `action` and `resource` use orchium's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.orchium` files is intentionally deferred until `.orchium` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether orchium permits an operation through its managed execution paths.
