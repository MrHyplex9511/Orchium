# Remove `packages/orchium/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/orchium/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the orchium legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/orchium/src/storage/db.ts` are concentrated in 22 source files:

- `packages/orchium/src/account/repo.ts`
- `packages/orchium/src/cli/cmd/db.ts`
- `packages/orchium/src/cli/cmd/import.ts`
- `packages/orchium/src/cli/cmd/stats.ts`
- `packages/orchium/src/control-plane/workspace.ts`
- `packages/orchium/src/index.ts`
- `packages/orchium/src/node.ts`
- `packages/orchium/src/permission/index.ts`
- `packages/orchium/src/project/project.ts`
- `packages/orchium/src/server/projectors.ts`
- `packages/orchium/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/orchium/src/server/shared/fence.ts`
- `packages/orchium/src/session/message-v2.ts`
- `packages/orchium/src/session/projectors.ts`
- `packages/orchium/src/session/prompt.ts`
- `packages/orchium/src/session/session.ts`
- `packages/orchium/src/session/todo.ts`
- `packages/orchium/src/share/share-next.ts`
- `packages/orchium/src/storage/db.ts`
- `packages/orchium/src/sync/index.ts`
- `packages/orchium/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy orchium database wrapper; `packages/orchium/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the orchium wrapper.
...
Status: Completed. `SyncEvent` and the orchium projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy orchium database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy orchium database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy orchium database wrapper.
...
All migration groups are complete or superseded. `packages/orchium/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No orchium data-migration group remains.

The previous orchium `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate orchium data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/orchium`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/orchium/src`
- `bun typecheck` from `packages/orchium`
- Relevant package tests from `packages/orchium`, not the repo root
