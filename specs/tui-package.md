...
Move the canonical Orchium terminal application from
`packages/orchium/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/orchium`,
`packages/cli`, or `@orchium/core`.

The SDK is the TUI's Orchium boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/orchium`.
- Do not replace private `packages/orchium` imports with `@orchium/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@orchium/tui`.
...
- Orchium domain data displayed by the TUI
...
```text
packages/orchium/src/cli/cmd/tui
```

Its private dependency on `packages/orchium` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/orchium/src/*`.
...
Status: Completed. The private `@orchium/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@orchium/tui`.
...
- The package has no dependency on `orchium`, `@orchium/cli`, or
  `@orchium/core`.
...
primitives now live in `@orchium/tui`. The legacy host consumes explicit
...
and build inputs through `@orchium/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Orchium globals or process state;
...
Replace process-global Orchium state with resolved TUI inputs.
...
`@orchium/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/orchium` initially.
...
canonical in `@orchium/tui`. Configured references resolve through the new
...
adapters rather than Orchium domain implementations.

Make the SDK the only Orchium domain boundary used by the TUI.
...
- Domain-facing TUI code imports Orchium data and operations only from
  `@orchium/sdk`.
...
`@orchium/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/orchium`.
...
- The package has no imports from `packages/orchium`, `packages/cli`, or
  `@orchium/core`.
...
public `@orchium/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/orchium`.
- Keep the legacy embedded worker and server startup in `packages/orchium`.
- Change those adapters to load config, create transport inputs, and call the
  public `@orchium/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/orchium`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@orchium/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@orchium/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Orchium domain state.
...
cd packages/orchium && bun typecheck
...
rg '@orchium/core|packages/orchium|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/orchium/src packages/orchium/test
```
...
- Build the current-platform `packages/orchium` binary.
