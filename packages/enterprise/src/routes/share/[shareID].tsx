import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@orchium/sdk/v2"
import { SessionTurn } from "@orchium/session-ui/session-turn"
import { SessionReview } from "@orchium/session-ui/session-review"
import { DataProvider } from "@orchium/session-ui/context"
import { FileComponentProvider } from "@orchium/ui/context/file"
import { WorkerPoolProvider } from "@orchium/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@orchium/ui/logo"
import { IconButton } from "@orchium/ui/icon-button"
import { ProviderIcon } from "@orchium/ui/provider-icon"
import { iife } from "@orchium/core/util/iife"
import { Binary } from "@orchium/core/util/binary"
import { NamedError } from "@orchium/core/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import NotFound from "../[...404]"
import { Tabs } from "@orchium/ui/tabs"
import { MessageNav } from "@orchium/session-ui/message-nav"
import { FileSSR } from "@orchium/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@orchium/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/orchium-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | Orchium</Title>
              </Show>
              <Meta name="description" content="orchium - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/MrHyplex9511/Orchium"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
