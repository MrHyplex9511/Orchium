...
import { PromptInput } from "@orchium/schema/prompt-input"
import { Session } from "@orchium/schema/session"
import { Project } from "@orchium/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@orchium/schema/schema"
...
import { Revert } from "@orchium/schema/revert"
import { SessionEvent } from "@orchium/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Orchium process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Orchium process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
