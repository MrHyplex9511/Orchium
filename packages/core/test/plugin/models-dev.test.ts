...
import { ModelsDevPlugin } from "@orchium/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.ORCHIUM_MODELS_PATH,
          disabled: Flag.ORCHIUM_DISABLE_MODELS_FETCH,
        }
        Flag.ORCHIUM_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.ORCHIUM_MODELS_PATH = previous.path
          Flag.ORCHIUM_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
