...
import { SessionCompaction } from "@orchium/core/session/compaction"
...
test("compaction prompt preserves detailed work state and relevant files", () => {
  ...
  expect(prompt.indexOf("## Current Work")).toBeLessThan(prompt.indexOf("## Work State"))
  expect(prompt.indexOf("## Work State")).toBeLessThan(prompt.indexOf("## Pending Tasks"))
  expect(prompt.indexOf("## Pending Tasks")).toBeLessThan(prompt.indexOf("## Next Move"))
  expect(prompt.indexOf("## Next Move")).toBeLessThan(prompt.indexOf("## Relevant Files"))
  expect(prompt).toContain("last command run, the last file being edited")
  expect(prompt).toContain("quoted verbatim from the most recent user message")
...
test("compaction prompt gives update instructions for a prior summary", () => {
  ...
  expect(prompt).toContain('Carry "Current Work" forward exactly as written')
