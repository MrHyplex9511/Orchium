...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@orchium/acme")).toBe("@orchium/acme")
    expect(Npm.sanitize("@orchium/acme@1.0.0")).toBe("@orchium/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/orchium/acme.git"
    const expected = win ? "acme@git+https_//github.com/orchium/acme.git" : spec
