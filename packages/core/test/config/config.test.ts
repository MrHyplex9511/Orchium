...
import { Config } from "@orchium/core/config"
import { ConfigProvider } from "@orchium/core/config/provider"
import { AppNodeBuilder } from "@orchium/core/effect/app-node-builder"
import { LayerNode } from "@orchium/core/effect/layer-node"
import { ConfigMigrateV1 } from "@orchium/core/v1/config/migrate"
...
import { Policy } from "@orchium/core/policy"
import { Project } from "@orchium/core/project"
import { AbsolutePath } from "@orchium/core/schema"
...
describe("Config", () => {
  ...
  it.live("loads orchium JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "orchium.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "orchium.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "orchium.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "orchium.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "orchium.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "orchium.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .orchium configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".orchium"), { recursive: true })
            await fs.mkdir(path.join(directory, ".orchium"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "orchium.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "orchium.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "orchium.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "orchium.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "orchium.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".orchium", "orchium.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".orchium", "orchium.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".orchium")),
              AbsolutePath.make(path.join(directory, ".orchium")),
            ])
