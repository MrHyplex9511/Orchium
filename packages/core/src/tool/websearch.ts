...
import { ToolFailure } from "@orchium/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@orchium/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.ORCHIUM_WEBSEARCH_PROVIDER === "exa" || process.env.ORCHIUM_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.ORCHIUM_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("ORCHIUM_EXPERIMENTAL") || truthy("ORCHIUM_ENABLE_EXA") || truthy("ORCHIUM_EXPERIMENTAL_EXA"),
    enableParallel: truthy("ORCHIUM_ENABLE_PARALLEL") || truthy("ORCHIUM_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `orchium/${InstallationVersion}`,
