...
> current private `@orchium/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@orchium/ai`
...
- Secondary audience: Orchium and other durable agent runtimes
...
import { LLM } from "@orchium/ai"
import { OpenAI } from "@orchium/ai/providers/openai"
...
```ts
import { Message } from "@orchium/ai"
...
import { LLM, Tool } from "@orchium/ai"
...
Orchium and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@orchium/ai/promise"
import { OpenAI } from "@orchium/ai/providers/openai"
...
```ts
import { LLMSchema } from "@orchium/ai/schema"
...
```ts
import { Provider, Protocol } from "@orchium/ai/provider"
...
@orchium/ai/providers/openai
@orchium/ai/providers/anthropic
@orchium/ai/providers/google
...
```
...
Orchium should migrate to `generateTurn` / `streamTurn`, preserving its durable
