---
title: Serwer
description: Interakcja z serwerem orchium za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `orchium serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient orchium.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `orchium.local` |
...
Ustaw `ORCHIUM_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `orchium` lub ustaw `ORCHIUM_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `orchium serve`, jak i `orchium web`.
...
Kiedy uruchomisz `orchium`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera orchium do programowej interakcji z orchium.
:::

Ta architektura umożliwia obsługę wielu klientów przez orchium i programową interakcję z kodem otwartym.

Możesz uruchomić `orchium serve`, aby uruchomić samodzielny serwer. Jeśli masz
orchium TUI działa, `orchium serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki orchium [IDE](/docs/ide).
...
Serwer orchium udostępnia następujące interfejsy API.
