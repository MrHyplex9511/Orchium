...
Umiejętności agenta pozwalają orchium odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
orchium przeszukuje te lokalizacje:

- Project config: `.orchium/skills/<name>/SKILL.md`
- Global config: `~/.config/orchium/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu orchium przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.orchium/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/orchium/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.orchium/skills/git-release/SKILL.md` w ten sposób:
...
orchium wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `orchium.json`:
...
**Dla agentów wbudowanych** (w `orchium.json`):
