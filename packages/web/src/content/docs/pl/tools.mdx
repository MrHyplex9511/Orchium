...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Orchium posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Orchium.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `ORCHIUM_EXPERIMENTAL_LSP_TOOL=true` (lub `ORCHIUM_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Orchium lub gdy zmienna środowiskowa `ORCHIUM_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Orchium:
