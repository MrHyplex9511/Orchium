---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć orchium.
---

Wtyczki umożliwiające wyłączenie orchium poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania orchium.
...
- `.orchium/plugins/` - Wtyczki na poziomie projektu
- `~/.config/orchium/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/orchium/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/orchium/orchium.json`)
2. Konfiguracja projektu (`orchium.json`)
3. Globalny katalog wtyczek (`~/.config/orchium/plugins/`)
4. Katalog wtyczek projektu (`.orchium/plugins/`)
...
orchium uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z orchium do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do orchium.
...
:::note
Jeśli korzystasz z aplikacji komputerowej orchium, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj orchium czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do orchium:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod orchium. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
