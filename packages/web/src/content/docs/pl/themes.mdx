...
Dzięki orchium możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie orchium używa naszego własnego motywu `orchium`.
...
orchium ma kilka wbudowanych motywów.
...
- Chcą, aby Orchium pasował do wyglądu ich terminala
...
orchium obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/orchium/themes/*.json` lub `$XDG_CONFIG_HOME/orchium/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.orchium/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.orchium/themes/*.json`
