...
Możesz dodać dodatkowe informacje do orchium za pomocą _Model Context Protocol_, w skrócie MCP. orchium obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [orchium Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/orchium`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
orchium automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, orchium:
...
Jeśli serwer wymaga uwierzytelnienia, orchium poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `orchium mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji orchium bezpieczne przechowa tokeny w `~/.local/share/orchium/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w orchium, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie orchium, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć orchium z kontem Sentry.
