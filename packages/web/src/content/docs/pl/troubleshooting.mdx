...
Aby debugować problemy z orchium, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\orchium\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `orchium --log-level DEBUG`.

---

## Przechowywanie danych

orchium przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/orchium/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\orchium`
...
orchium Desktop uruchamia w tle lokalny serwer orchium (sidecar `orchium-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Orchium` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/orchium/orchium.jsonc` (lub `~/.config/orchium/orchium.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/orchium/orchium.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\orchium\orchium.jsonc`
...
orchium może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/orchium/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\orchium\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby orchium mógł ją odbudować.

1. Całkowicie zamknij orchium Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/orchium`
- **Linux**: usuń `~/.cache/orchium` (lub uruchom `rm -rf ~/.cache/orchium`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\orchium`

3. Uruchom ponownie orchium Desktop.

---

### Napraw problemy z połączeniem z serwerem

orchium Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `orchium.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `ORCHIUM_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `ORCHIUM_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows orchium Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami orchium.

---

### Brak powiadomień

orchium Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla orchium w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie orchium Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji orchium Desktop):

- `orchium.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `orchium.global.dat` i `orchium.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `orchium upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `orchium models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\orchium`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. orchium dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\orchium`

2. Uruchom ponownie kod orchium, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to orchium do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
orchium wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
