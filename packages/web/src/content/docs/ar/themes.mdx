...
مع Orchium يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Orchium سمتنا `orchium`.
...
يأتي Orchium مع عدة سمات مدمجة.
...
- يريدون أن يطابق Orchium مظهر terminal لديهم
...
يدعم Orchium نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/orchium/themes/*.json` أو `$XDG_CONFIG_HOME/orchium/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.orchium/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.orchium/themes/*.json`
