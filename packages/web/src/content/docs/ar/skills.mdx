...
تتيح مهارات الوكيل لـ Orchium اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Orchium في هذه المواقع:

- إعدادات المشروع: `.orchium/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/orchium/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Orchium صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.orchium/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/orchium/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.orchium/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Orchium المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `orchium.json`:
...
**للوكلاء المدمجين** (في `orchium.json`):
