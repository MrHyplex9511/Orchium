---
title: الخادم
description: تفاعل مع خادم orchium عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `orchium serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل orchium استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `orchium.local` |
...
عيّن `ORCHIUM_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `orchium`، أو عيّن `ORCHIUM_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `orchium serve` و `orchium web`.
...
عند تشغيل `orchium` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم orchium للتفاعل مع orchium برمجيا.
:::

تتيح هذه البنية لـ orchium دعم عدة عملاء وتمكّنك من التفاعل مع orchium برمجيا.

يمكنك تشغيل `orchium serve` لبدء خادم مستقل. إذا كانت واجهة orchium في terminal (TUI)
قيد التشغيل، فسيبدأ `orchium serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Orchium لـ [IDE](/docs/ide).
...
يعرض خادم orchium واجهات API التالية.
