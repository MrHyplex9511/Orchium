---
title: TUI
description: استخدام واجهة المستخدم TUI في Orchium.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يوفّر Orchium واجهة terminal تفاعلية (TUI) للعمل على مشاريعك باستخدام LLM.

يؤدي تشغيل Orchium إلى بدء واجهة TUI للدليل الحالي.
...
عند استخدام واجهة Orchium في terminal، يمكنك كتابة `/` متبوعة باسم أمر لتنفيذ الإجراءات بسرعة. مثلا:
...
أضف موفّرا إلى Orchium. يتيح لك اختيار أحد الموفّرين المتاحين وإضافة مفاتيح API الخاصة بهم.
...
اخرج من Orchium. _الأسماء المستعارة_: `/quit`, `/q`
...
هذا منفصل عن `orchium.json`، الذي يضبط سلوك الخادم ووقت التشغيل.
...
- `leader_timeout` - يتحكم في مدة انتظار Orchium بعد الضغط على leader key. القيمة الافتراضية `2000`.
...
- `sound_pack` - sound pack ID المراد استخدامه. القيمة الافتراضية `orchium.default`.
