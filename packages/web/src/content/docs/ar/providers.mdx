---
title: المزوّدون
description: استخدام أي موفّر LLM في Orchium.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Orchium ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Orchium.
...
في `~/.local/share/orchium/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Orchium.
...
Orchium Zen هي قائمة نماذج يوفّرها فريق Orchium وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Orchium. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Orchium Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر orchium، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/auth).
...
يعمل مثل أي مزوّد آخر في Orchium واستخدامه اختياري بالكامل.

---

## Orchium Go

Orchium Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Orchium، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Orchium.

1. شغّل الأمر `/connect` في TUI، واختر `Orchium Go`، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع Orchium:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل orchium:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `orchium.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Orchium غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين orchium لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل orchium بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات orchium.
...
تخزين الرمز في مساحة تخزين مصادقة orchium.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Orchium نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Orchium
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`orchium.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [orchium-gitlab-auth](https://www.npmjs.com/package/orchium-gitlab-auth).

##### التهيئة

خصّص عبر `orchium.json`:
...
لاستخدام اشتراك GitHub Copilot مع orchium:
...
لاستخدام Google Vertex AI مع Orchium:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر orchium، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [orchium-helicone-session](https://github.com/H2Shami/orchium-helicone-session) لتسجيل كل محادثة Orchium تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Orchium مدرجة كجلسة مستقلة.
...
يمكنك ضبط orchium لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط orchium لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط orchium لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Orchium. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/orchium) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Orchium:
...
3. انسخ مفتاح API لاستخدامه في Orchium.
...
6. **مهم**: قبل استخدام نماذج السحابة في Orchium، يجب سحب معلومات النموذج محليا:
...
Orchium Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Orchium. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Orchium Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Orchium Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Orchium.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات orchium.
...
5. يمكنك أيضا تخصيصها عبر إعدادات orchium. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Orchium:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات orchium. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع orchium. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `orchium.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Orchium بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `orchium auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات orchium وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات orchium.
