---
title: Zen
description: قائمة منتقاة من النماذج يوفّرها Orchium.
---
...
Orchium Zen هي قائمة بالنماذج التي اختبرها فريق Orchium وتحقّق منها.

يعمل Zen مثل أي مزوّد آخر في Orchium. تسجّل الدخول إلى Orchium Zen وتحصل على
مفتاح API. استخدامه **اختياري بالكامل** ولا تحتاج إليه لاستخدام Orchium.
...
:::tip
اختبرنا مجموعة منتقاة من النماذج والمزوّدين الذين يعملون جيدا مع Orchium.
:::
...
Orchium Zen هي بوابة AI تتيح لك الوصول إلى هذه النماذج.

---

## كيف يعمل

يعمل Orchium Zen مثل أي مزوّد آخر في Orchium.

1. تسجّل الدخول إلى **<a href={console}>Orchium Zen</a>**، وتضيف تفاصيل الفوترة، ثم تنسخ مفتاح API.
2. تشغّل الأمر `/connect` في واجهة TUI، وتختار Orchium Zen، ثم تلصق مفتاح API.
...
يستخدم [معرّف النموذج](/docs/config/#models) في إعدادات Orchium الصيغة `orchium/<model-id>`. على سبيل المثال، بالنسبة إلى GPT 5.5، ستستخدم `orchium/gpt-5.5` في إعداداتك.
...
قد تلاحظ [نماذج منخفضة التكلفة](/docs/config/#models)، مثل Haiku أو Nano أو Flash، في سجل الاستخدام. يستخدم Orchium هذه النماذج لإنشاء عناوين الجلسات.
...
- MiMo-V2.5 Free متاح على Orchium لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Ling 3.0 Flash Fin Free متاح على Orchium لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Nemotron 3 Ultra Free متاح على Orchium لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Nemotron 3.5 Lightning Free متاح على Orchium لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Big Pickle نموذج خفي ومتاح مجانا على Orchium لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Muse Spark 1.3 Contributor Free متاح على Orchium لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
...
4. **عدم فرض أي ارتباط حصري** عبر تمكينك من استخدامه مع أي عامل برمجة آخر. ومع إبقاء إمكانية استخدام أي مزوّد آخر مع Orchium أيضا دائما.
