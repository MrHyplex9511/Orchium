...
أنشئ الملف `.orchium/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Orchium أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Orchium:
...
- على مستوى النظام: `~/.config/orchium/commands/`
- لكل مشروع: `.orchium/commands/`
...
يتضمن orchium عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
