...
description: Orchium, LSP sunucularınızla bütünleşir.
---

Orchium, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Orchium, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`ORCHIUM_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve orchium bir dosyayı açtığında:
...
LSP sunucularını, orchium yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
