---
title: Sunucu
description: orchium sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`orchium serve` komutu, orchium istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `orchium.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `ORCHIUM_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `orchium` değeridir; değiştirmek isterseniz `ORCHIUM_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `orchium serve` hem de `orchium web` için geçerlidir.
...
`orchium` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
orchium ile programatik etkileşim için orchium sunucusunu kullanın.
:::

Bu mimari, orchium'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`orchium serve` ile bağımsız bir sunucu başlatabilirsiniz. orchium TUI açıksa, `orchium serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum orchium [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
orchium sunucusu aşağıdaki API'leri sunar.
