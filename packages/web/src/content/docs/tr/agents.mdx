...
orchium'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
orchium, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Orchium, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Orchium iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `orchium.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse orchium modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global orchium yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
orchium yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Orchium Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `orchium/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `orchium models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/MrHyplex9511/Orchium).
:::
