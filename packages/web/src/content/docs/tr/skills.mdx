...
Ajan becerileri, orchium'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
orchium su konumlari tarar:

- Proje konfigurasyonu: `.orchium/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/orchium/skills/<name>/SKILL.md`
...
Proje yerel yollarinda orchium, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.orchium/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/orchium/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.orchium/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
orchium, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`orchium.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`orchium.json` icinde):
