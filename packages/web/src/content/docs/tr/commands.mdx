...
`.orchium/commands/test.md` oluştur:
...
orchium işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

orchium [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/orchium/commands/`
- Proje başına: `.orchium/commands/`
...
orchium, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
