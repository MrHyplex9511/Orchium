...
_Model Bağlam Protokolü_ veya MCP kullanarak orchium'a harici araçlar ekleyebilirsiniz. orchium hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [orchium Config](https://opencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/orchium` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
orchium, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde orchium şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa orchium, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `orchium mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından orchium, belirteçleri `~/.local/share/orchium/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra orchium'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi orchium yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve orchium'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
