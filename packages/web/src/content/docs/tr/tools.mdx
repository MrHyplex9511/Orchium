...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. orchium yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
orchium içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `ORCHIUM_EXPERIMENTAL_LSP_TOOL=true` (veya `ORCHIUM_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca orchium provider kullanılırken veya `ORCHIUM_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

orchium başlatırken etkinleştirmek için:
