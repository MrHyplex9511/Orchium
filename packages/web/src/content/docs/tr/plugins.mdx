---
title: Eklentiler
description: Orchium'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Orchium'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Orchium'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.orchium/plugins/` - Proje düzeyinde eklentiler
- `~/.config/orchium/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/orchium/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/orchium/orchium.json`)
2. Project config (`orchium.json`)
3. Global eklenti dizini (`~/.config/orchium/plugins/`)
4. Proje eklenti dizini (`.orchium/plugins/`)
...
Orchium bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Orchium SDK istemcisi.
...
Orchium'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Orchium masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Orchium'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Orchium'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Orchium'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Orchium'a da hazır olacaktır.
