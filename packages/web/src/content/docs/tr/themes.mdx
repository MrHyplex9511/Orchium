...
orchium ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak orchium kendi `orchium` temasını kullanır.
...
orchium birden fazla yerleşik temayla gelir.
...
- orchium'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
orchium, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/orchium/themes/*.json` veya `$XDG_CONFIG_HOME/orchium/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.orchium/themes/*.json`
4. **Geçerli çalışma dizini** - `./.orchium/themes/*.json`
