...
Create `.orchium/commands/test.md`:
...
You can add custom commands through the Orchium config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Orchium [config](/docs/config):
...
orchium includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
