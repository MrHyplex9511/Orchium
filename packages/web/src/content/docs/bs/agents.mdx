...
Postoje dvije vrste agenata u Orchium: primarni agenti i podagenti.
...
Orchium dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Orchium dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Orchium dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u Orchium-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `orchium.json`:
...
- Globalno: `~/.config/orchium/agents/`
- Po projektu: `.orchium/agents/`

```markdown title="~/.config/orchium/agents/review.md"
---
...
Ako temperatura nije navedena, Orchium koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Orchium konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Orchium konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Orchium Zen](/docs/zen), koristili biste `orchium/gpt-5.1-codex` za GPT 5.1 Codex.
...
```json title="orchium.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="orchium.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Pokrenite `orchium models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/MrHyplex9511/Orchium).
:::
...
```markdown title="~/.config/orchium/agents/docs-writer.md"
---
...
```markdown title="~/.config/orchium/agents/security-auditor.md"
---
