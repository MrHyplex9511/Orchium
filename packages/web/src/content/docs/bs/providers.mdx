---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Orchium.
---

import config from "../../../../config.mjs"
export const console = config.console

Orchium koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Orchium konfiguraciji.
...
u `~/.local/share/orchium/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Orchium
config.
...
Orchium Zen je lista modela koje je obezbedio Orchium tim koji su bili
testirano i potvrđeno da dobro radi sa Orchium. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Orchium Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite orchium i idite na [opencode.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u Orchium i potpuno je opcionalan za korištenje.

---

## Orchium Go

Orchium Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Orchium tim i koji su testirani i verificirani da dobro rade s Orchium-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Orchium Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s Orchium:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete orchium:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `orchium.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Orchium.
:::
...
Možete konfigurirati orchium za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi orchium ispravno radio.
   :::
...
Također možete dodati modele kroz svoju orchium konfiguraciju.
...
da pohrani token u orchium auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Orchium koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Orchium
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`orchium.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [orchium-gitlab-auth](https://www.npmjs.com/package/orchium-gitlab-auth).

##### Konfiguracija

Prilagodite putem `orchium.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s orchium:
...
Za korištenje Google Vertex AI s Orchium:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće orchium.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem orchiuma, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [orchium-helicone-session](https://github.com/H2Shami/orchium-helicone-session) da automatski prijavite svaki Orchium razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Orchium razgovor naveden kao zasebna sesija.
...
Možete konfigurirati orchium za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati orchium za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati orchium za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Orchium. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/orchium) za detalje.
:::
...
Da biste koristili Ollama Cloud s Orchium:
...
3. Kopirajte API ključ za korištenje u Orchium.
...
6. **Važno**: Prije upotrebe modela oblaka u Orchium, morate lokalno povući informacije o modelu:
...
Orchium Zen je lista testiranih i verifikovanih modela koju je obezbedio Orchium tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Orchium Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Orchium Zen**.

```txt
   /connect
```

3. Unesite svoj Orchium API ključ.
...
Također možete dodati dodatne modele putem vaše orchium konfiguracije.
...
5. Također ih možete prilagoditi putem vaše orchium konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Orchiumom:
...
Također možete prilagoditi modele kroz svoju orchium konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s orchium-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `orchium.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Orchium da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `orchium auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Orchium konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj orchium konfiguraciji.
