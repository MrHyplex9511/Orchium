...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Orchium dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Orchium.
...
:::note
Ovaj alat je dostupan samo kada je `ORCHIUM_EXPERIMENTAL_LSP_TOOL=true` (ili `ORCHIUM_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Orchium provajdera ili kada je varijabla `ORCHIUM_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Orchium:
