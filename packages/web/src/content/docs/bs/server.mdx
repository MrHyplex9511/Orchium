---
title: Server
description: Komunicirajte s orchium serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `orchium serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji orchium klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `orchium.local` |
...
Postavite `ORCHIUM_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `orchium`, ili postavite `ORCHIUM_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `orchium serve` i za `orchium web`.
...
Kada pokrenete `orchium`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite orchium server za programsku interakciju sa orchium-om.
:::

Ova arhitektura omogucava orchium podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `orchium serve` da startate standalone server. Ako je orchium TUI vec pokrenut, `orchium serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Orchium [IDE](/docs/ide) pluginovi.
...
orchium server izlaže sljedece API-je.
