...
Možete dodati vanjske alate u Orchium koristeći _Model Context Protocol_, ili MCP. Orchium podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Orchium Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/orchium`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Orchium automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Orchium će:
...
Ako server zahtijeva autentifikaciju, Orchium će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `orchium mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Orchium sigurno čuva tokene u `~/.local/share/orchium/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Orchium, zajedno s ugrađenim alatima. Možete njima upravljati kroz Orchium konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Orchium sa vašim Sentry nalogom.
