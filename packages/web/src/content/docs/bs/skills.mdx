...
Agent skills omogucavaju Orchium da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Orchium pretrazuje ove lokacije:

- Konfiguracija projekta: `.orchium/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/orchium/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Orchium ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.orchium/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/orchium/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.orchium/skills/git-release/SKILL.md` ovako:
...
Orchium navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `orchium.json`:
...
**Za ugradene agente** (u `orchium.json`):
