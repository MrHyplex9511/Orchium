...
Da biste otklonili probleme s Orchium, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\orchium\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `orchium --log-level DEBUG`.

---

## Pohrana

Orchium pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/orchium/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\orchium`
...
Orchium Desktop pokreće lokalni Orchium server (`orchium-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Orchium` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/orchium/orchium.jsonc` (ili `~/.config/orchium/orchium.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/orchium/orchium.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\orchium\orchium.jsonc`
...
Orchium također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/orchium/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\orchium\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Orchium mogao ponovo izgraditi.

1. Potpuno zatvorite Orchium Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/orchium`
- **Linux**: obrišite `~/.cache/orchium` (ili pokrenite `rm -rf ~/.cache/orchium`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\orchium`

3. Ponovo pokrenite Orchium Desktop.

---

### Rješavanje problema sa vezom na serveru

Orchium Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `orchium.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `ORCHIUM_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `ORCHIUM_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Orchium Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Orchium karakteristikama.

---

### Obavještenja se ne prikazuju

Orchium Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Orchium u postavkama vašeg OS-a, i
...
1. Zatvorite Orchium Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Orchium Desktop aplikacije):

- `orchium.settings.dat` (URL zadanog servera za desktop)
- `orchium.global.dat` i `orchium.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `orchium models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\orchium`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Orchium dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\orchium`

2. Ponovo pokrenite Orchium da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Orchium da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Orchium će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
