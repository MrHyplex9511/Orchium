...
U Orchium mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Orchium koristi nasu `orchium` temu.
...
Orchium dolazi sa vise ugradenih tema.
...
- Zele da Orchium odgovara izgledu njihovog terminala
...
Orchium podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/orchium/themes/*.json` ili `$XDG_CONFIG_HOME/orchium/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.orchium/themes/*.json`
4. **Trenutni radni direktorij** - `./.orchium/themes/*.json`
