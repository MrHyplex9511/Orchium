---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Orchium.
---

Dodaci vam omogućavaju da proširite Orchium spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Orchium.
...
- `.orchium/plugins/` - Dodaci na nivou projekta
- `~/.config/orchium/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/orchium/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/orchium/orchium.json`)
2. Konfiguracija projekta (`orchium.json`)
3. Globalni direktorij dodataka (`~/.config/orchium/plugins/`)
4. Direktorij dodataka projekta (`.orchium/plugins/`)
...
Orchium pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Orchium SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Orchium.
...
Spriječite orchium da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u orchium:
...
Pomoćnik `tool` kreira prilagođeni alat koji orchium može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za orchium zajedno sa ugrađenim alatima.
