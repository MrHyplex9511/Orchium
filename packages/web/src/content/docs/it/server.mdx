---
title: Server
description: Interagisci con il server orchium via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `orchium serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client orchium.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `orchium.local` |
...
Imposta `ORCHIUM_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `orchium`, oppure imposta `ORCHIUM_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `orchium serve` sia per `orchium web`.
...
Quando esegui `orchium` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server orchium per interagire con orchium in modo programmatico.
:::

Questa architettura permette a orchium di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `orchium serve` per avviare un server standalone. Se la TUI di orchium e gia in esecuzione, `orchium serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Orchium.
...
Il server orchium espone le seguenti API.
