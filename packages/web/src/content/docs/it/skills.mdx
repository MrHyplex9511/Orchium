...
Le skill degli agenti permettono a Orchium di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Orchium cerca in queste posizioni:

- Config di progetto: `.orchium/skills/<name>/SKILL.md`
- Config globale: `~/.config/orchium/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Orchium risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.orchium/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/orchium/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.orchium/skills/git-release/SKILL.md` cosi':
...
Orchium elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `orchium.json`:
...
**Per agenti integrati** (in `orchium.json`):
