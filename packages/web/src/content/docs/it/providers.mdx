---
title: Provider
description: Usa qualsiasi provider LLM in Orchium.
---

import config from "../../../../config.mjs"
export const console = config.console

Orchium usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Orchium.
...
Orchium Zen e una lista di modelli forniti dal team Orchium, testati e
verificati per funzionare bene con Orchium. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Orchium Zen.
:::

1. Esegui `/connect` nella TUI, seleziona orchium e vai su [opencode.ai/auth](https://opencode.ai/auth).
...
Funziona come qualsiasi altro provider in Orchium ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Orchium:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui orchium:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `orchium.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Orchium non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare orchium per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché orchium funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di orchium.
...
salvare il token nello storage di auth di orchium.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Orchium usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Orchium
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `orchium.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [orchium-gitlab-auth](https://www.npmjs.com/package/orchium-gitlab-auth).

##### Configurazione

Personalizza tramite `orchium.json`:
...
Per usare il tuo abbonamento GitHub Copilot con orchium:
...
Per usare Google Vertex AI con Orchium:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui orchium.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da orchium, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [orchium-helicone-session](https://github.com/H2Shami/orchium-helicone-session) per loggare automaticamente ogni conversazione di Orchium come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Orchium elencata come sessione separata.
...
Puoi configurare orchium per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare orchium per usare modelli locali tramite LM Studio.
...
Puoi configurare orchium per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Orchium. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/orchium) per dettagli.
:::
...
Per usare Ollama Cloud con Orchium:
...
3. Copia la chiave API da usare in Orchium.
...
6. **Importante**: Prima di usare modelli cloud in Orchium, devi scaricare le informazioni del modello localmente:
...
Orchium Zen è una lista di modelli testati e verificati forniti dal team Orchium. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Orchium Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Orchium Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Orchium.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di orchium.
...
5. Puoi anche personalizzarli tramite la configurazione di orchium. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Orchium:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di orchium. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con orchium. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `orchium.json` nella directory del progetto:
...
I campi `limit` permettono a Orchium di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `orchium auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di orchium e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config orchium.
