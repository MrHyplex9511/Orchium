---
title: Plugin
description: Scrivi plugin per estendere Orchium.
---

I plugin ti permettono di estendere Orchium agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Orchium.
...
- `.orchium/plugins/` - plugin a livello progetto
- `~/.config/orchium/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/orchium/node_modules/`.
...
1. Config globale (`~/.config/orchium/orchium.json`)
2. Config di progetto (`orchium.json`)
3. Directory plugin globale (`~/.config/orchium/plugins/`)
4. Directory plugin di progetto (`.orchium/plugins/`)
...
Orchium esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di orchium per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere orchium.
...
:::note
Se usi l'app desktop di Orchium, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a orchium di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a orchium:
...
L'helper `tool` crea uno strumento personalizzato che orchium può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in orchium insieme agli strumenti integrati.
