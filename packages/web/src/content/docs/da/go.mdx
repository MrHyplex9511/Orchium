...
Orchium Go er et lavprisabonnement til **$10/måned**, der giver dig pålidelig adgang til populære åbne kodningsmodeller.

Go fungerer som enhver anden udbyder i Orchium. Du abonnerer på Orchium Go og
...
:::tip
Vi testede en udvalgt gruppe af modeller og udbydere, der fungerer godt med Orchium.
:::
...
Orchium Go giver dig adgang til disse modeller for **$10/måned**.

---

## Sådan fungerer det

Orchium Go fungerer som enhver anden udbyder i Orchium.

1. Du logger ind på **<a href={console}>Orchium Zen</a>**, abonnerer på Go, og
   kopierer din API-nøgle.
2. Du kører kommandoen `/connect` i TUI'en, vælger `Orchium Go`, og indsætter
   din API-nøgle.
3. Kør `/models` i TUI'en for at se listen over tilgængelige modeller gennem Go.

:::note
Kun ét medlem per arbejdsområde kan abonnere på Orchium Go.
:::
...
Orchium Go er udviklet til [Orchium](https://opencode.ai) og andre kodningsagenter,
...
3. Sende et stabilt sessions-id i `x-orchium-session` for hver samtale, så vi kan optimere routing og
   prompt-caching.

### Validerede klienter

Ud over Orchium er følgende klienter blevet valideret til at fungere korrekt
med Orchium Go. Vi garanterer dog ikke, at de fortsat vil fungere fremover.

| Klient            | Sessionsunderstøttelse                                                                                                                                                                                                                                    |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | Builds, der indeholder [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864), sender headeren i primære og sekundære Orchium-anmodninger. Rettelsen blev merged efter v0.21.0; den version indeholder den ikke i sig selv.              |
...
| **ZCode**         | Go genkender dens indbyggede sessionsheader. Vores [anmodning om `x-orchium-session`](https://github.com/zai-org/feedback/issues/492) er stadig åben, men det er ikke længere nødvendigt at sende netop denne header.                                    |
| **Pi**            | Aktuelle builds sender sessionsoplysninger til Orchium. Opdater ældre installationer.                                                                                                                                                                    |
...
| **Kilo Code CLI** | Builds, der indeholder [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752), gendanner Orchium-sessionsheadere. Denne rettelse dækker CLI'en, men ikke VS Code-udvidelsen. Se [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
Dit [model id](/docs/config/#models) i din Orchium config
bruger formatet `orchium-go/<model-id>`. For eksempel for Kimi K3, vil du
bruge `orchium-go/kimi-k3` i din config.
...
4. Undgå **lock-in** ved også at lade dig bruge enhver anden udbyder med Orchium.
