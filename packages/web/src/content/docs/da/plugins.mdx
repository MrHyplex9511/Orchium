---
title: Plugins
description: Skriv dine egne plugins for at udvide Orchium.
---

Plugins giver dig mulighed for at udvide Orchium ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Orchiums standardadfærd.
...
- `.orchium/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/orchium/node_modules/`.
...
1. Global konfiguration (`~/.config/orchium/orchium.json`)
2. Projektkonfiguration (`orchium.json`)
3. Global plugin-mappe (`~/.config/orchium/plugins/`)
4. Projekt plugin bibliotek (`.orchium/plugins/`)
...
Orchium kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En orchium SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide orchium.
...
:::note
Hvis du bruger Orchium desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå orchium i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til orchium:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som orchium kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for orchium sammen med indbyggede værktøjer.
