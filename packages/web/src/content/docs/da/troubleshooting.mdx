...
For at fejlfinde problemer med Orchium, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\orchium\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `orchium --log-level DEBUG`.

---

## Lagring

orchium gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/orchium/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\orchium`
...
Orchium Desktop kører en lokal Orchium-server (`orchium-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Orchium`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/orchium/orchium.jsonc` (eller `~/.config/orchium/orchium.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/orchium/orchium.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\orchium\orchium.jsonc`
...
Orchium kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/orchium/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\orchium\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Orchium kan genopbygge den.

1. Afslut Orchium Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/orchium`
- **Linux**: slet `~/.cache/orchium` (eller kør `rm -rf ~/.cache/orchium`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\orchium`

3. Start Orchium Desktop på ny.

---

### Løs problemer med serverforbindelse

Orchium Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `orchium.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `ORCHIUM_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `ORCHIUM_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Orchium Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Orchium.

---

### Meddelelser vises ikke

Orchium Desktop viser kun systemvarsler når:

- varsler er aktiveret for Orchium i dine OS-indstillinger, og
...
1. Afslut Orchium Desktop.
2. Find og slet disse filer (de findes i Orchium Desktop-appens datamappe):

- `orchium.settings.dat` (skrivebordsstandardserver URL)
- `orchium.global.dat` og `orchium.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Orchium:
...
For at finde ud af hvilke modeller du har adgang til, kør `orchium models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\orchium`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. orchium installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\orchium`

2. Start orchium på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge orchium til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
orchium vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
