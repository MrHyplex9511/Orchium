...
Med Orchium kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Orchium vårt eget `orchium`-tema.
...
Orchium kommer med flere innebygde temaer.
...
- Vil at Orchium skal matche terminalens utseende
...
Orchium støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/orchium/themes/*.json` eller `$XDG_CONFIG_HOME/orchium/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.orchium/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.orchium/themes/*.json`
