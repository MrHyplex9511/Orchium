...
Der er to typer agenter i Orchium; primære agenter og subagenter.
...
Orchium leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
Orchium leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

Orchium leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i Orchium's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `orchium.json`-konfigurationsfil:
...
- Globalt: `~/.config/orchium/agents/`
- Pr. projekt: `.orchium/agents/`
...
Hvis ingen temperatur er angivet, bruger Orchium modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale Orchium-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din Orchium-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [Orchium Zen](/docs/zen), vil du bruge `orchium/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `orchium models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/MrHyplex9511/Orchium).
:::
