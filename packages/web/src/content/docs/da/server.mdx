---
title: Server
description: Interager med orchium-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `orchium serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en orchium-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `orchium.local` |
...
Indstil `ORCHIUM_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `orchium`, eller indstil `ORCHIUM_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `orchium serve` og `orchium web`.
...
Når du kører `orchium` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug orchium-serveren til at interagere med orchium programmatisk.
:::

Denne arkitektur lader orchium understøtte flere klienter og lader dig interagere med orchium programmatisk.

Du kan køre `orchium serve` for at starte en selvstændig server. Hvis du har
orchium TUI kørende, vil `orchium serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Orchium [IDE](/docs/ide) plugins.
...
Orchium-serveren viser følgende API'er.
