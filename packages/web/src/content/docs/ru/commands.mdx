...
Создайте `.orchium/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию orchium или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем orchium [config](/docs/config):
...
- Глобальный: `~/.config/orchium/commands/`
- Для каждого проекта: `.orchium/commands/`
...
orchium включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
