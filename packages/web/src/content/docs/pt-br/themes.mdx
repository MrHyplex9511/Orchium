...
Com o orchium, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o orchium usa nosso próprio tema `orchium`.
...
O orchium vem com vários temas embutidos.
...
- Querem que o orchium corresponda à aparência do seu terminal
...
O orchium suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/orchium/themes/*.json` ou `$XDG_CONFIG_HOME/orchium/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.orchium/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.orchium/themes/*.json`
