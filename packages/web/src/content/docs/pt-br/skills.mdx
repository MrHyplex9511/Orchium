...
As habilidades do agente permitem que o orchium descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O orchium pesquisa nesses locais:

- Configuração do projeto: `.orchium/skills/<name>/SKILL.md`
- Configuração global: `~/.config/orchium/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o orchium sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.orchium/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/orchium/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.orchium/skills/git-release/SKILL.md` assim:
...
O orchium lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `orchium.json`:
...
**Para agentes embutidos** (em `orchium.json`):
