---
title: Servidor
description: Interaja com o servidor orchium via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `orchium serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente orchium pode usar.
...
```bash
orchium serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `orchium.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
orchium serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `ORCHIUM_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `orchium`, ou defina `ORCHIUM_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `orchium serve` quanto ao `orchium web`.

```bash
ORCHIUM_SERVER_PASSWORD=your-password orchium serve
```
...
Quando você executa `orchium`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor orchium para interagir com o orchium programaticamente.
:::

Essa arquitetura permite que o orchium suporte múltiplos clientes e permite que você interaja com o orchium programaticamente.

Você pode executar `orchium serve` para iniciar um servidor autônomo. Se você tiver o TUI do orchium em execução, `orchium serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do orchium [IDE](/docs/ide).
...
O servidor orchium expõe as seguintes APIs.
