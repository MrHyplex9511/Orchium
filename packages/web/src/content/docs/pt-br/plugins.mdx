---
title: Plugins
description: Escreva seus próprios plugins para estender o orchium.
---

Plugins permitem que você estenda o orchium conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do orchium.
...
- `.orchium/plugins/` - Plugins em nível de projeto
- `~/.config/orchium/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/orchium/node_modules/`.
...
1. Configuração global (`~/.config/orchium/orchium.json`)
2. Configuração do projeto (`orchium.json`)
3. Diretório de plugins global (`~/.config/orchium/plugins/`)
4. Diretório de plugins do projeto (`.orchium/plugins/`)
...
O orchium executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do orchium para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o orchium.
...
:::note
Se você estiver usando o aplicativo desktop orchium, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o orchium de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao orchium:
...
O helper `tool` cria uma ferramenta personalizada que o orchium pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o orchium junto com as ferramentas integradas.
