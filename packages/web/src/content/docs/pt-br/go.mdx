...
O Orchium Go é uma assinatura de baixo custo de **US$ 10/mês** que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Orchium. Você assina o Orchium Go e
...
O Orchium Go lhe dá acesso a esses modelos por **US$ 10/mês**.

---

## Como funciona

O Orchium Go funciona como qualquer outro provedor no Orchium.

1. Você entra no **<a href={console}>Orchium Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Orchium Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Orchium Go.
:::
...
O Orchium Go foi projetado para o [Orchium](https://opencode.ai) e outros agentes de programação
...
3. Enviar um ID de sessão estável em `x-orchium-session` para cada conversa, para que possamos otimizar o roteamento e
   o cache de prompts.

### Clientes validados

Além do Orchium, os clientes a seguir foram validados para funcionar corretamente
com o Orchium Go. No entanto, não garantimos que continuarão funcionando no futuro.

| Cliente           | Suporte a sessões                                                                                                                                                                                                                                                     |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | As builds que contêm o [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) enviam o header nas requisições principais e auxiliares do Orchium. A correção foi incorporada após a v0.21.0; essa versão por si só não a inclui.                     |
...
| **ZCode**         | O Go reconhece seu header de sessão nativo. Nossa [solicitação de `x-orchium-session`](https://github.com/zai-org/feedback/issues/492) continua aberta, mas não é mais necessário enviar especificamente esse header.                                                |
| **Pi**            | As builds atuais enviam informações de sessão para o Orchium. Atualize instalações mais antigas.                                                                                                                                                                     |
...
| **Kilo Code CLI** | As builds que contêm o [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) restauram os headers de sessão do Orchium. Essa correção abrange a CLI, não a extensão do VS Code. Consulte a [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
O [ID do modelo](/docs/config/#models) na sua configuração do Orchium
usa o formato `orchium-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`orchium-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Orchium também.
