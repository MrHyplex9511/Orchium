...
Tools allow the LLM to perform actions in your codebase. Orchium comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Orchium.
...
:::note
This tool is only available when `ORCHIUM_EXPERIMENTAL_LSP_TOOL=true` (or `ORCHIUM_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the Orchium or Orchium Go provider, or when either the `ORCHIUM_ENABLE_EXA` or `ORCHIUM_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Orchium:

```bash
ORCHIUM_ENABLE_EXA=1 orchium
# or
ORCHIUM_ENABLE_PARALLEL=1 orchium
```
