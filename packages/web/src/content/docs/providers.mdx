---
title: Providers
description: Using any LLM provider in Orchium.
---

import config from "../../../config.mjs"
export const console = config.console

Orchium uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Orchium config.
...
in `~/.local/share/orchium/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Orchium
config.
...
Orchium Zen is a list of models provided by the Orchium team that have been
tested and verified to work well with Orchium. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Orchium Zen.
:::

1. Run the `/connect` command in the TUI, select `Orchium Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in Orchium and is completely optional to use.

---

## Orchium Go

Orchium Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Orchium team that have been
tested and verified to work well with Orchium.

1. Run the `/connect` command in the TUI, select `Orchium Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with Orchium:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running orchium:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY orchium

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile orchium

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX orchium
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `orchium.json`:

   ```json title="orchium.json"
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "amazon-bedrock": {
...
:::warning
Using your Claude Pro/Max (or other Claude.ai consumer subscription) with
Orchium via OAuth is **not supported by Anthropic** and can violate their
[Terms of Service](https://claude.com/docs/claude-science/legal-and-compliance).

Anthropic states that OAuth authentication is intended exclusively for
purchasers of its subscription plans for use with **native Anthropic
applications**, and reserves the right to enforce these restrictions **without
prior notice**. Users have reported **account suspension and loss of paid
subscription** after using Claude Pro/Max subscriptions through third-party
tools such as this one.

The supported way to use Anthropic models with Orchium is an **API key**
(usage-based billing) — see "API Key" above.
:::

:::info
There are plugins that allow you to use your Claude Pro/Max models with
Orchium. Anthropic explicitly prohibits this.

Previous versions of Orchium came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Orchium with zero setup:
...
You can configure orchium to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="orchium.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for orchium to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX orchium
   ```
...
Orchium does not query Azure management APIs or discover deployments. Select a model whose catalog name matches your deployment, or configure its deployment name explicitly:
...
Assign your identity the inference role required by the deployment: **Cognitive Services OpenAI User** for Azure OpenAI models or **Cognitive Services User** for other Foundry models. Orchium refreshes access tokens through the Azure CLI, including versions earlier than 2.54.0, so you only need to sign in again when the CLI session expires.
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX orchium
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your orchium config.
...
Orchium supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Orchium uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Orchium. Sign in and approve.

   :::note
   Orchium requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Orchium surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Orchium forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Orchium is experimental. Features, configuration, and
behavior may change in future releases.
:::

Orchium integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in orchium auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Orchium uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Orchium
to only use your own GitLab-hosted instance, add the following to your
`orchium.json` file. It is also recommended to disable session sharing.
...
More documentation on [orchium-gitlab-auth](https://www.npmjs.com/package/orchium-gitlab-auth) homepage.

##### Configuration

Customize through `orchium.json`:
...
When a `duo-workflow-*` model is selected, Orchium will:
...
4. Route tool execution requests through Orchium's permission-gated tool system
...
To use your GitHub Copilot subscription with orchium:
...
To use Google Vertex AI with Orchium:
...
2. Set the required environment variables:
   ...
   Set them while running orchium.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id orchium
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through orchium, you can always configure it yourself.
...
```jsonc title="~/.config/orchium/orchium.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [orchium-helicone-session](https://github.com/H2Shami/orchium-helicone-session) plugin to automatically log each Orchium conversation as a session in Helicone.

```bash
npm install -g orchium-helicone-session
```
...
```json title="orchium.json"
{
  "plugin": ["orchium-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Orchium conversation listed as a separate session.
...
You can configure orchium to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="orchium.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure orchium to use local models through LM Studio.

```json title="orchium.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure orchium to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Orchium. See the [Ollama integration docs](https://docs.ollama.com/integrations/orchium) for details.
:::

```json title="orchium.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Orchium:
...
3. Copy the API key for use in Orchium.
...
6. **Important**: Before using cloud models in Orchium, you must pull the model information locally:
...
Orchium Zen is a list of tested and verified models provided by the Orchium team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Orchium Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Orchium Zen**.

   ```txt
   /connect
   ```

3. Enter your Orchium API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your orchium config.
...
5. You can also customize them through your orchium config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' orchium
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group orchium
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Orchium:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=orchium&utm_medium=docs&utm_campaign=cortex-provider).

orchium's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. Orchium captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting orchium:
...
The model catalog is provided automatically. A minimal `orchium.json` is all that's needed:
...
You can also customize models through your orchium config. Here's an example of specifying provider routing order.
...
2. Select **SuperGrok Subscription**. Orchium opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by Orchium. Orchium polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
Orchium refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
:::tip
You can use any OpenAI-compatible provider with orchium. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in orchium.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `orchium.json` file in your project directory:

   ```json title="orchium.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="orchium.json" {9,11,17-20}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Orchium to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `orchium auth list` to see if the credentials
...
2. For custom providers, check the orchium config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your orchium config.
