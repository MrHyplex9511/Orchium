...
With Orchium you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Orchium uses our own `orchium` theme.
...
Orchium comes with several built-in themes.
...
- Want Orchium to match their terminal's appearance
...
Orchium supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/orchium/themes/*.json` or `$XDG_CONFIG_HOME/orchium/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.orchium/themes/*.json`
4. **Current working directory** - Defined in `./.orchium/themes/*.json`
