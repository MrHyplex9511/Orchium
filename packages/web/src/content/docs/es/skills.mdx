...
Las habilidades del agente permiten a Orchium descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Orchium busca estas ubicaciones:

- Configuración del proyecto: `.orchium/skills/<name>/SKILL.md`
- Configuración global: `~/.config/orchium/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Orchium sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.orchium/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/orchium/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.orchium/skills/git-release/SKILL.md` así:
...
Orchium enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `orchium.json`:
...
**Para agentes integrados** (en `orchium.json`):
