...
Con Orchium puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Orchium usa nuestro propio tema `orchium`.
...
Orchium viene con varios temas integrados.
...
- Quiere que Orchium coincida con la apariencia de su terminal
...
Orchium admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/orchium/themes/*.json` o `$XDG_CONFIG_HOME/orchium/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.orchium/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.orchium/themes/*.json`
