...
Para depurar problemas con Orchium, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/orchium/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\orchium\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `orchium --log-level DEBUG`.

---

## Almacenamiento

orchium almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/orchium/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\orchium`
...
Orchium Desktop ejecuta un servidor Orchium local (el sidecar `orchium-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Orchium` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/orchium/orchium.jsonc` (o `~/.config/orchium/orchium.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/orchium/orchium.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\orchium\orchium.jsonc`
...
Orchium también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/orchium/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\orchium\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.orchium/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Orchium pueda reconstruirlo.

1. Salga de Orchium Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/orchium`
- **Linux**: eliminar `~/.cache/orchium` (o ejecutar `rm -rf ~/.cache/orchium`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\orchium`

3. Reinicie el escritorio Orchium.

---

### Solucionar problemas de conexión del servidor

Orchium Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `orchium.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `ORCHIUM_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `ORCHIUM_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Orchium requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Orchium.

---

### Notificaciones que no se muestran

Orchium Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Orchium en la configuración de su sistema operativo, y
...
1. Salga del escritorio Orchium.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Orchium):

- `orchium.settings.dat` (URL del servidor predeterminado de escritorio)
- `orchium.global.dat` y `orchium.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Orchium:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/MrHyplex9511/Orchium/issues**](https://github.com/MrHyplex9511/Orchium/issues)
...
3. Asegúrese de tener la última versión con `orchium upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `orchium models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/orchium
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\orchium`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. orchium instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\orchium`

2. Reinicie orchium para reinstalar los paquetes del proveedor más recientes.

Esto obligará a orchium a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
orchium detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
