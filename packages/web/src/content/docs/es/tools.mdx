...
Las herramientas permiten que LLM realice acciones en su código base. Orchium viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Orchium.
...
:::note
Esta herramienta solo está disponible cuando `ORCHIUM_EXPERIMENTAL_LSP_TOOL=true` (o `ORCHIUM_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Orchium o cuando la variable de entorno `ORCHIUM_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Orchium:

```bash
ORCHIUM_ENABLE_EXA=1 orchium
```
