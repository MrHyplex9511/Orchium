---
title: Complementos
description: Escriba sus propios complementos para extender Orchium.
---

Los complementos le permiten extender Orchium al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Orchium.
...
- `.orchium/plugins/` - Complementos a nivel de proyecto
- `~/.config/orchium/plugins/` - Complementos globales
...
```json title="orchium.json"
{
  ...
  "plugin": ["orchium-helicone-session", "orchium-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/orchium/node_modules/`.
...
1. Configuración global (`~/.config/orchium/orchium.json`)
2. Configuración del proyecto (`orchium.json`)
3. Directorio global de complementos (`~/.config/orchium/plugins/`)
4. Directorio de complementos del proyecto (`.orchium/plugins/`)
...
```json title=".orchium/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Orchium ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".orchium/plugins/my-plugin.ts"
...
```js title=".orchium/plugins/example.js"
...
- `client`: Un cliente SDK orchium para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar orchium.
...
```js title=".orchium/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "orchium"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Orchium, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que orchium lea archivos `.env`:

```javascript title=".orchium/plugins/env-protection.js"
...
```javascript title=".orchium/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a orchium:

```ts title=".orchium/plugins/custom-tools.ts"
import { type Plugin, tool } from "@orchium/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que orchium puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para orchium junto con las herramientas integradas.
...
```ts title=".orchium/plugins/compaction.ts"
import type { Plugin } from "@orchium/plugin"
...
```ts title=".orchium/plugins/custom-compaction.ts"
import type { Plugin } from "@orchium/plugin"
