...
description: Cree herramientas que LLM pueda llamar en orchium.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de orchium como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.orchium/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/orchium/tools/`.
...
```ts title=".orchium/tools/database.ts" {1}
...
```ts title=".orchium/tools/math.ts"
import { tool } from "@orchium/plugin"
...
```ts title=".orchium/tools/bash.ts"
import { tool } from "@orchium/plugin"
...
```ts title=".orchium/tools/project.ts" {8}
import { tool } from "@orchium/plugin"
...
```python title=".orchium/tools/add.py"
import sys
...
```ts title=".orchium/tools/python-add.ts" {10}
import { tool } from "@orchium/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".orchium/tools/add.py")
