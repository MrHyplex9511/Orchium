...
Puede agregar herramientas externas a Orchium utilizando el _Model Context Protocol_ o MCP. Orchium admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Orchium Config](https://opencode.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/orchium`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Orchium maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Orchium:
...
Si el servidor requiere autenticación, Orchium le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `orchium mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Orchium almacenará los tokens de forma segura en `~/.local/share/orchium/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Orchium, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Orchium como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Orchium a su cuenta Sentry.
