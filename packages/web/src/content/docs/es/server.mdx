---
title: Servidor
description: Interactuar con el servidor orchium a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `orchium serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente orchium puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `orchium.local` |
...
Configure `ORCHIUM_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `orchium`, o configure `ORCHIUM_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `orchium serve` como a `orchium web`.
...
Cuando ejecuta `orchium`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor orchium para interactuar con orchium mediante programación.
:::

Esta arquitectura permite que orchium admita múltiples clientes y le permite interactuar con orchium mediante programación.

Puede ejecutar `orchium serve` para iniciar un servidor independiente. Si tienes el
orchium TUI ejecutándose, `orchium serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Orchium [IDE](/docs/ide).
...
El servidor orchium expone las siguientes API.
