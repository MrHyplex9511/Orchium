---
title: Server
description: Interact with orchium server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `orchium serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an orchium client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `orchium.local` |
...
Set `ORCHIUM_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `orchium`, or set `ORCHIUM_SERVER_USERNAME` to override it. This applies to both `orchium serve` and `orchium web`.
...
When you run `orchium` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the orchium server to interact with orchium programmatically.
:::

This architecture lets orchium support multiple clients and allows you to interact with orchium programmatically.

You can run `orchium serve` to start a standalone server. If you have the
orchium TUI running, `orchium serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Orchium [IDE](/docs/ide) plugins.
...
The orchium server exposes the following APIs.
