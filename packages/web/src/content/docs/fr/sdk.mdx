---
title: SDK
description: Client JS de type sécurisé pour le serveur orchium.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK orchium JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler orchium par programme.
...
Créez une instance de orchium :

```javascript
import { createOrchium } from "@orchium/sdk"

const { client } = await createOrchium()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `orchium.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${orchium.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de orchium, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@orchium/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*orchium" },
})
