---
title: Plugins
description: Écrivez vos propres plugins pour étendre Orchium.
---

Les plugins vous permettent d'étendre Orchium en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Orchium.
...
- `.orchium/plugins/` - Plugins au niveau du projet
- `~/.config/orchium/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/orchium/node_modules/`.
...
1. Configuration globale (`~/.config/orchium/orchium.json`)
2. Configuration du projet (`orchium.json`)
3. Répertoire global des plugins (`~/.config/orchium/plugins/`)
4. Répertoire des plugins du projet (`.orchium/plugins/`)
...
Orchium exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK orchium pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre orchium.
...
:::note
Si vous utilisez l'application de bureau Orchium, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez orchium de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à orchium :
...
L'assistant `tool` crée un outil personnalisé que orchium peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour orchium aux côtés des outils intégrés.
