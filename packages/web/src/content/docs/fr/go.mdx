...
Orchium Go est un abonnement à bas coût à **10 $/mois** qui vous donne un accès fiable aux modèles de codage ouverts populaires.

Go fonctionne comme n'importe quel autre fournisseur dans Orchium. Vous vous abonnez à Orchium Go et obtenez votre clé d'API. C'est **totalement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser Orchium.
...
Orchium Go vous donne accès à ces modèles pour **10 $/mois**.

---

## Comment ça marche

Orchium Go fonctionne comme n'importe quel autre fournisseur dans Orchium.

1. Vous vous connectez à **<a href={console}>Orchium Zen</a>**, vous vous abonnez à Go et copiez votre clé d'API.
2. Vous exécutez la commande `/connect` dans la TUI, sélectionnez `Orchium Go` et collez votre clé d'API.
3. Exécutez `/models` dans la TUI pour voir la liste des modèles disponibles via Go.

:::note
Un seul membre par espace de travail peut s'abonner à Orchium Go.
:::
...
Orchium Go est conçu pour [Orchium](https://opencode.ai) et d'autres agents de codage
...
3. Envoyer un ID de session stable dans `x-orchium-session` pour chaque conversation afin que nous puissions optimiser le routage et
   la mise en cache des prompts.

### Clients validés

Outre Orchium, le bon fonctionnement des clients suivants avec Orchium Go a été
...
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | Les builds contenant la [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) envoient l'en-tête sur les requêtes Orchium principales et auxiliaires. Le correctif a été fusionné après la v0.21.0 ; cette version seule ne l'inclut pas.               |
...
| **ZCode**         | Go reconnaît son en-tête de session natif. Notre [demande concernant `x-orchium-session`](https://github.com/zai-org/feedback/issues/492) reste ouverte, mais il n'est plus nécessaire d'envoyer spécifiquement cet en-tête.                                             |
| **Pi**            | Les builds actuels envoient les informations de session pour Orchium. Mettez à jour les installations plus anciennes.                                                                                                                                                    |
...
| **Kilo Code CLI** | Les builds contenant la [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) rétablissent les en-têtes de session Orchium. Ce correctif concerne la CLI, pas l'extension VS Code. Consultez l'[issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
L'[ID de modèle](/docs/config/#models) dans votre configuration Orchium utilise le format `orchium-go/<model-id>`. Par exemple, pour Kimi K3, vous utiliseriez `orchium-go/kimi-k3` dans votre configuration.
...
Nous avons créé Orchium Go pour :
...
4. N'avoir **aucun verrouillage exclusif** en vous permettant d'utiliser n'importe quel autre fournisseur avec Orchium également.
