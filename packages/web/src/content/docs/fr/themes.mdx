...
Avec Orchium, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Orchium utilise notre propre thème `orchium`.
...
Orchium est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Orchium corresponde à l'apparence de leur terminal
...
Orchium prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/orchium/themes/*.json` ou `$XDG_CONFIG_HOME/orchium/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.orchium/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.orchium/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/orchium/themes
vim ~/.config/orchium/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .orchium/themes
vim .orchium/themes/my-theme.json
```
