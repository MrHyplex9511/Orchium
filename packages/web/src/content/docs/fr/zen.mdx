---
title: Zen
description: Liste organisée de modèles fournis par Orchium.
---
...
Orchium Zen est une liste de modèles testés et vérifiés fournie par l'équipe Orchium.

Zen fonctionne comme n'importe quel autre fournisseur dans Orchium. Vous vous connectez à Orchium Zen et obtenez votre clé API. C'est **entièrement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser Orchium.
...
:::tip
Nous avons testé un groupe sélectionné de modèles et de fournisseurs qui fonctionnent bien avec Orchium.
:::
...
Orchium Zen est une passerelle AI qui vous donne accès à ces modèles.

---

## Comment ça marche

Orchium Zen fonctionne comme n'importe quel autre fournisseur dans Orchium.

1. Vous vous connectez à **<a href={console}>Orchium Zen</a>**, ajoutez vos informations de facturation et copiez votre clé API.
2. Vous exécutez la commande `/connect` dans le TUI, sélectionnez Orchium Zen et collez votre clé API.
...
Le [model id](/docs/config/#models) dans votre configuration Orchium utilise le format `orchium/<model-id>`. Par exemple, pour GPT 5.5, vous utiliseriez `orchium/gpt-5.5` dans votre configuration.
...
Vous remarquerez peut-être des [modèles à faible coût](/docs/config/#models), tels que Haiku, Nano ou Flash, dans votre historique d'utilisation. Orchium utilise ces modèles pour générer les titres des sessions.
...
- MiMo-V2.5 Free est disponible sur Orchium pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Ling 3.0 Flash Fin Free est disponible sur Orchium pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Nemotron 3 Ultra Free est disponible sur Orchium pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Nemotron 3.5 Lightning Free est disponible sur Orchium pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Big Pickle est un modèle stealth gratuit sur Orchium pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Muse Spark 1.3 Contributor Free est disponible sur Orchium pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
...
Nous avons créé Orchium Zen pour :
...
4. N'avoir **aucun lock-in** en vous permettant de l'utiliser avec n'importe quel autre agent de codage. Et toujours vous permettre d'utiliser n'importe quel autre fournisseur avec Orchium également.
