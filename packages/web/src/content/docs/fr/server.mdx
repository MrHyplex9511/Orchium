---
title: Serveur
description: Interagissez avec le serveur orchium via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `orchium serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client orchium peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `orchium.local` |
...
Définissez `ORCHIUM_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `orchium`, ou définissez `ORCHIUM_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `orchium serve` et à `orchium web`.
...
Lorsque vous exécutez `orchium`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur orchium pour interagir avec orchium par programmation.
:::

Cette architecture permet à orchium de prendre en charge plusieurs clients et vous permet d'interagir avec orchium par programme.

Vous pouvez exécuter `orchium serve` pour démarrer un serveur autonome. Si vous avez le
orchium TUI en cours d'exécution, `orchium serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Orchium [IDE](/docs/ide).
...
Le serveur orchium expose les API suivantes.
