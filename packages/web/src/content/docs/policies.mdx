---
title: Policies
description: Control which configured resources Orchium may use.
---

Policies control whether Orchium may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `orchium.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether Orchium may use a resource such as an LLM provider.
...
Orchium currently supports one policy action:
