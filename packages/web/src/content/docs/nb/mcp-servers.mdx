...
Du kan legge til eksterne verktøy i Orchium ved å bruke _Model Context Protocol_ eller MCP. Orchium støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Orchium Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="orchium.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/orchium`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Orchium håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Orchium:
...
Hvis serveren krever autentisering, vil Orchium be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `orchium mcp auth <server-name>`.
...
```bash
orchium mcp auth my-oauth-server
```
...
```bash
orchium mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Orchium lagre tokens sikkert i `~/.local/share/orchium/mcp-auth.json`.
...
orchium mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Orchium, sammen med innebygde verktøy. Så du kan administrere dem gjennom Orchium-konfigurasjonen som alle andre verktøy.
...
```json title="orchium.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
orchium mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Orchium til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Orchium-samtale oppført som en separat økt.
