...
description: Skriv dine egne programtillegg for å utvide Orchium.
---

Plugins lar deg utvide Orchium ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Orchium.
...
- `.orchium/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/orchium/node_modules/`.
...
1. Global konfigurasjon (`~/.config/orchium/orchium.json`)
2. Prosjektkonfigurasjon (`orchium.json`)
3. Global plugin-katalog (`~/.config/orchium/plugins/`)
4. Prosjektpluginkatalog (`.orchium/plugins/`)
...
Orchium kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Orchium SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Orchium.
...
:::note
Hvis du bruker Orchium-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Orchium fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Orchium:
...
`tool`-hjelperen lager et tilpasset verktøy som Orchium kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Orchium sammen med innebygde verktøy.
