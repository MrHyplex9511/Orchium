...
Agentferdigheter lar Orchium oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
orchium søker etter disse stedene:

- Prosjektkonfigurasjon: `.orchium/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/orchium/skills/<name>/SKILL.md`
...
For prosjektlokale stier går orchium opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.orchium/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/orchium/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.orchium/skills/git-release/SKILL.md` slik:
...
orchium viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `orchium.json`:
...
**For innebygde agenter** (i `orchium.json`):
