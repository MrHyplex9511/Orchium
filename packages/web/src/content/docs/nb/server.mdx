---
title: Tjener
description: Samhandle med Orchium-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `orchium serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Orchium-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `orchium.local` |
...
Sett `ORCHIUM_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `orchium` som standard, eller sett `ORCHIUM_SERVER_USERNAME` for å overstyre det. Dette gjelder både `orchium serve` og `orchium web`.
...
Når du kjører `orchium` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Orchium-serveren til å samhandle med Orchium programmatisk.
:::

Denne arkitekturen lar Orchium støtte flere klienter og lar deg samhandle med Orchium programmatisk.

Du kan kjøre `orchium serve` for å starte en frittstående server. Hvis du har
Orchium TUI kjører, vil `orchium serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Orchium [IDE](/docs/ide) plugins.
...
orchium-serveren viser følgende APIer.
