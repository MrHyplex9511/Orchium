---
title: TUI
description: Bruke orchium-terminalbrukergrensesnittet.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Orchium gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre Orchium starter TUI for gjeldende katalog.
...
Når du bruker Orchium TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til Orchium. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt Orchium. _Aliaser_: `/quit`, `/q`
...
Dette er atskilt fra `orchium.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge Orchium venter etter leader key. Standard er `2000`.
...
Bruk `ORCHIUM_TUI_CONFIG` for å laste en egendefinert TUI-konfigurasjonsbane.
...
- `sound_pack` - Sound pack-ID som skal brukes. Standard er `orchium.default`.
