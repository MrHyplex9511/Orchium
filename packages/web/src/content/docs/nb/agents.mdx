...
Det er to typer agenter i Orchium; primære agenter og underagenter.
...
Orchium kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Orchium kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Orchium kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `orchium.json`:
...
Hvis ingen temperatur er spesifisert, bruker Orchium modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Orchium-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Orchium-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Orchium Zen](/docs/zen), vil du bruke `orchium/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `orchium models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/MrHyplex9511/Orchium).
:::
