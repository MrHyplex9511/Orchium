---
title: Orchium Zen
description: Orchium によって提供されるモデルの厳選されたリスト。
---
...
Orchium Zen は、Orchium チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

Zen は Orchium のほかのプロバイダーと同じように動作します。Orchium Zen にログインして API キーを取得できます。これは **完全にオプション** であり、Orchium を使うために必須ではありません。
...
:::tip
Orchium でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
Orchium Zen は、これらのモデルにアクセスできる AI ゲートウェイです。

---

## 仕組み

Orchium Zen は、Orchium のほかのプロバイダーと同じように動作します。

1. **<a href={console}>Orchium Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、Orchium Zen を選んで API キーを貼り付けます。
...
Orchium 設定で使う [model id](/docs/config/#models) は `orchium/<model-id>` 形式です。たとえば、GPT 5.5 では設定に `orchium/gpt-5.5` を使用します。
...
使用履歴に Haiku、Nano、Flash などの[低コストモデル](/docs/config/#models)が表示されることがあります。Orchium はこれらのモデルをセッションタイトルの生成に使用します。
...
- MiMo-V2.5 Free は期間限定で Orchium で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Ling 3.0 Flash Fin Free は期間限定で Orchium で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3 Ultra Free は期間限定で Orchium で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3.5 Lightning Free は期間限定で Orchium で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Big Pickle はステルスモデルで、期間限定で Orchium で無料提供されています。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Muse Spark 1.3 Contributor Free は期間限定で Orchium で利用できます。チームはこの期間を活用してフィードバックを収集し、モデルを改善しています。
...
私たちが Orchium Zen を作った目的は次のとおりです。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに Orchium では常にほかのプロバイダーも使えるようにすること。
