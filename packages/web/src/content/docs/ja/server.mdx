---
title: サーバー
description: HTTP 経由で Orchium サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`orchium serve` コマンドは、Orchium クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `orchium.local` |
...
HTTP 基本認証でサーバーを保護するには、`ORCHIUM_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `orchium` になるか、`ORCHIUM_SERVER_USERNAME` を設定してオーバーライドします。これは、`orchium serve` と `orchium web` の両方に当てはまります。
...
`orchium` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Orchium サーバーを使用して、プログラムで Orchium と対話します。
:::

`orchium serve` を実行してスタンドアロンサーバーを起動できます。すでに Orchium TUI を実行している場合でも、`orchium serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Orchium [IDE](/docs/ide) プラグインによって使用されます。
