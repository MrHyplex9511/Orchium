---
title: Regeln
description: Lege eigene Anweisungen fuer orchium fest.
---

Du kannst orchium ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in orchium den Befehl `/init` aus.
...
So kann orchium sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht orchium sie zu erweitern.
...
orchium kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/orchium/AGENTS.md` speichern.
Diese gelten in allen orchium-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Orchium dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/orchium/AGENTS.md` existiert)
...
Beim Start sucht orchium in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/orchium/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `orchium.json` oder in `~/.config/orchium/orchium.json` angeben.
...
orchium parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit orchium.json

Empfohlen ist das `instructions`-Feld in der `orchium.json`:
...
Du kannst orchium explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `orchium.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
