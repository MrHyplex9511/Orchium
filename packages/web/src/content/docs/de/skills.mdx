...
Agent Skills erlauben Orchium, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Orchium sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Orchium vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.orchium/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/orchium/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.orchium/skills/git-release/SKILL.md` zum Beispiel so:
...
Orchium listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `orchium.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `orchium.json`):
