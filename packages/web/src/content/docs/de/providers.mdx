---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Orchium.
---

import config from "../../../../config.mjs"
export const console = config.console

Orchium verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Orchium-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Orchium anpassen
config.
...
Orchium Zen ist eine Liste von Modellen, die vom Orchium-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Orchium funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Orchium Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Orchium aus und gehen Sie zu [opencode.ai/auth](https://opencode.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in Orchium und ist völlig optional.

---

## Orchium Go

Orchium Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Orchium-Team getestet und verifiziert wurden, dass sie gut mit Orchium funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Orchium Go` und gehen Sie zu [opencode.ai/auth](https://opencode.ai/zen).
...
So verwenden Sie Amazon Bedrock mit Orchium:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Orchium ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `orchium.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Orchium wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können orchium so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Orchium ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Orchium-Konfiguration hinzufügen.
...
um das Token im Orchium-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Orchium verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Orchium
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`orchium.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [orchium-gitlab-auth](https://www.npmjs.com/package/orchium-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `orchium.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Orchium:
...
So verwenden Sie Google Vertex AI mit Orchium:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Orchium ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Orchium konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [orchium-helicone-session](https://github.com/H2Shami/orchium-helicone-session)-Plugin, um jede Orchium-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Orchium-Konversation als separate Sitzung aufgeführt.
...
Sie können Orchium für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Orchium über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Orchium über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Orchium konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/orchium).
:::
...
So verwenden Sie Ollama Cloud mit Orchium:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Orchium.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Orchium verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Orchium Zen ist eine Liste getesteter und verifizierter Modelle, die vom Orchium-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Orchium Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Orchium Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Orchium API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Orchium-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Orchium-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Orchium:
...
Sie können Modelle auch über Ihre Orchium-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Orchium verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `orchium.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Orchium erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `orchium auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Orchium-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Orchium-Konfiguration übereinstimmt.
