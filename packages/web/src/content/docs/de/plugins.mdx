---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Orchium zu erweitern.
---

Mit Plugins können Sie Orchium erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Orchium zu ändern.
...
- `.orchium/plugins/` – Plugins auf Projektebene
- `~/.config/orchium/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/orchium/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/orchium/orchium.json`)
2. Projektkonfiguration (`orchium.json`)
3. Globales Plugin-Verzeichnis (`~/.config/orchium/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.orchium/plugins/`)
...
Orchium führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Orchium-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Orchium erweitern können.
...
:::note
Wenn Sie die Orchium-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Orchium `.env`-Dateien liest:
...
Plugins können Orchium auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Orchium aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Orchium zur Verfügung.
