...
Erstellen Sie `.orchium/commands/test.md`:

```md title=".orchium/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die Orchium-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem Orchium [config](/docs/config):

```json title="orchium.jsonc" {4-12}
{
...
- Global: `~/.config/orchium/commands/`
- Pro Projekt: `.orchium/commands/`

```markdown title="~/.config/orchium/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".orchium/commands/component.md"
---
...
```md title=".orchium/commands/create-file.md"
---
...
```md title=".orchium/commands/analyze-coverage.md"
---
...
```md title=".orchium/commands/review-changes.md"
---
...
```md title=".orchium/commands/review-component.md"
---
...
Orchium enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
