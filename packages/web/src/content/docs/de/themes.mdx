...
Mit Orchium kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Orchium das `orchium`-Theme.
...
Orchium liefert mehrere eingebaute Themes mit.
...
- Orchium optisch ans Terminal anpassen wollen
...
Orchium unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/orchium/themes/*.json` oder `$XDG_CONFIG_HOME/orchium/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.orchium/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.orchium/themes/*.json`
