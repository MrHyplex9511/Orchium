---
title: Server
description: Nutze den orchium-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `orchium serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein orchium-Client nutzen kann.
...
Setze `ORCHIUM_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `orchium`, kann aber mit `ORCHIUM_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `orchium serve` und `orchium web`.
...
Wenn du `orchium` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den orchium-Server, um orchium programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `orchium serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `orchium serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Orchium-[IDE](/docs/ide)-Plugins.
...
Der orchium-Server stellt folgende APIs bereit.
