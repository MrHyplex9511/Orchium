...
Wenn Orchium Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\orchium\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `orchium --log-level DEBUG`.

---

## Speicher

orchium speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/orchium/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\orchium`
...
Orchium Desktop startet im Hintergrund einen lokalen Orchium-Server (`orchium-cli`-Sidecar).
...
- Nur macOS: `Orchium`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/orchium/orchium.jsonc` (oder `~/.config/orchium/orchium.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/orchium/orchium.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\orchium\orchium.jsonc`
...
Orchium kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/orchium/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\orchium\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Orchium ihn neu aufbauen kann.

1. Beende Orchium Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/orchium`
- **Linux**: loesche `~/.cache/orchium` (oder fuehre aus: `rm -rf ~/.cache/orchium`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\orchium`

3. Starte Orchium Desktop neu.

---

### Server-Verbindungsprobleme beheben

Orchium Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `orchium.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `ORCHIUM_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `ORCHIUM_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Orchium Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Orchium-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Orchium Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Orchium in den OS-Einstellungen aktiviert sind, und
...
1. Beende Orchium Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Orchium Desktop):

- `orchium.settings.dat` (desktop default server URL)
- `orchium.global.dat` und `orchium.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Orchium hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `orchium models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\orchium`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. orchium installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\orchium`

2. Starte orchium neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt orchium, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
orchium erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
