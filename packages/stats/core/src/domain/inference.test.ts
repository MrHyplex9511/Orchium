...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve orchium route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "orchium")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "orchium")).toBe("openai")
    expect(statProvider("big-pickle", "", "orchium")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "orchium"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep orchium as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "orchium"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "orchium"))).toMatchObject([{ provider: "unknown" }])
    expect(toProviderAggregate(aggregate("muse-spark-1.2-contributor", "unknown"))).toMatchObject([
      { provider: "meta" },
    ])
  })

  test("geo aggregates never keep orchium or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "orchium"), country: "US" })).toMatchObject([
