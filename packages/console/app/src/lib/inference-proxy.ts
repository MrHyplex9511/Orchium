...
import { and, Database, eq, isNull, sql } from "@orchium/console-core/drizzle/index.js"
...
export async function proxyInference(
  ...
  const requestID = request.headers.get("x-orchium-request-id") ?? request.headers.get("x-orchium-request")
  if (requestID) forwarded.headers.set("x-orchium-request-id", requestID)
