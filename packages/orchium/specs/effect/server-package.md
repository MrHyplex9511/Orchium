...
Practical reference for a future `packages/server` split after the orchium
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/orchium`.
...
Until enough shared service code lives outside `packages/orchium`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/orchium`

It should not import `packages/orchium` services while `packages/orchium`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on orchium-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/orchium` implementation details.
