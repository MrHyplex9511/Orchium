...
Delete `packages/orchium/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/orchium/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/orchium/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/orchium`
- TUI plugin loader tests in `packages/orchium` if runtime plugin API wiring changed
