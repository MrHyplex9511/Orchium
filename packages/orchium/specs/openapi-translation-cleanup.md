...
Trim `packages/orchium/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/orchium/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/orchium`.
- `[x]` `bun typecheck` from `packages/orchium`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/orchium/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/orchium`.
- `[x]` `bun dev generate > /tmp/orchium-openapi.json` from `packages/orchium`.
...
- Edit `packages/orchium/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/orchium-openapi.json` from `packages/orchium`.
...
- Check source schemas in files like `packages/orchium/src/session/schema.ts`, `packages/orchium/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/orchium/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/orchium/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
