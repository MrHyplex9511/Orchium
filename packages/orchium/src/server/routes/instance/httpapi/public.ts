...
import { OrchiumHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for orchium serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(OrchiumHttpApi)
...
export const PublicApi = OrchiumHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "orchium",
    version: "1.0.0",
    description: "orchium api",
