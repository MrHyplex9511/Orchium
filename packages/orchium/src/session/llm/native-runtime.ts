...
import type { LLMClientShape } from "@orchium/llm/route"
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @orchium/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains orchium-owned. The native runtime only adapts
      // the @orchium/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
