...
export const DEPRECATED_PLUGIN_PACKAGES = ["orchium-openai-codex-auth", "orchium-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, orchiumVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(orchiumVersion) || semver.major(orchiumVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.orchium
  if (typeof range !== "string") return
  if (!semver.satisfies(orchiumVersion, range)) {
    throw new Error(`Plugin requires orchium ${range} but running ${orchiumVersion}`)
  }
}
