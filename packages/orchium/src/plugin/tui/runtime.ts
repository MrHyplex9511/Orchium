...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@orchium/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@orchium/core/effect/app-node-builder"
import { errorData, errorMessage } from "@orchium/tui/util/error"
...
import { hasTheme, upsertTheme } from "@orchium/tui/context/theme"
...
import type { HostPluginApi, HostSlots } from "@orchium/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@orchium/core/v1/config/plugin"
import { createCommandShim } from "@orchium/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@orchium/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".orchium"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".orchium", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".orchium", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.ORCHIUM_PURE ? [] : pluginOrigins
    if (Flag.ORCHIUM_PURE && pluginOrigins.length) {
    }
