...
import { serviceUse } from "@orchium/core/effect/service-use"
...
import { InstallationLocal, InstallationVersion } from "@orchium/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@orchium/core/v1/config/console-state"
...
import { RemoteAuthError } from "@orchium/core/v1/config/error"
import { ConfigPermissionV1 } from "@orchium/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@orchium/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["orchium.jsonc", "orchium.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.ORCHIUM_CONFIG && !Flag.ORCHIUM_CONFIG_DIR && !Flag.ORCHIUM_CONFIG_CONTENT) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "orchium.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "orchium.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "ORCHIUM_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/orchium`
        ...
        if (Flag.ORCHIUM_CONFIG) {
          yield* merge(Flag.ORCHIUM_CONFIG, yield* loadFile(Flag.ORCHIUM_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.ORCHIUM_CONFIG })
        }

        if (!Flag.ORCHIUM_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("orchium", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.ORCHIUM_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from ORCHIUM_CONFIG_DIR", { path: Flag.ORCHIUM_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".orchium") || dir === Flag.ORCHIUM_CONFIG_DIR) {
            for (const file of ["orchium.json", "orchium.jsonc"]) {
          ...
          // Auto-discovered plugins under `.orchium/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.ORCHIUM_CONFIG_CONTENT) {
          const source = "ORCHIUM_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.ORCHIUM_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from ORCHIUM_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["ORCHIUM_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("ORCHIUM_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.ORCHIUM_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.ORCHIUM_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("ORCHIUM_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.ORCHIUM_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.ORCHIUM_DISABLE_PRUNE) {
