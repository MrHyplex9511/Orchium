...
import { isRecord } from "@orchium/tui/util/record"
...
import { TuiKeybind } from "@orchium/tui/config/keybind"
...
import { TuiConfig } from "@orchium/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@orchium/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old orchium.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.orchium`
  // folders between cwd and home, and ORCHIUM_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.ORCHIUM_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit ORCHIUM_TUI_CONFIG override, if set.
  if (Flag.ORCHIUM_TUI_CONFIG) {
    const configFile = Flag.ORCHIUM_TUI_CONFIG
  ...
  // 4. `.orchium` directories (and ORCHIUM_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".orchium") || dir === Flag.ORCHIUM_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".orchium") && dir !== Flag.ORCHIUM_CONFIG_DIR) continue
