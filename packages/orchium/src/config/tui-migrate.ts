...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from orchium.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const orchium = await orchiumFiles(input)
  for (const file of orchium) {
...
async function orchiumFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "orchium"),
    ...(await Filesystem.findUp(["orchium.json", "orchium.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "orchium"))
  }
  if (Flag.ORCHIUM_CONFIG) files.push(Flag.ORCHIUM_CONFIG)
