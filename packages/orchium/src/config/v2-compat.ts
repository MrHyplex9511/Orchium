...
import { NonNegativeInt, PositiveInt } from "@orchium/core/schema"
import { ConfigAttachmentV1 } from "@orchium/core/v1/config/attachment"
import { ConfigLSPV1 } from "@orchium/core/v1/config/lsp"
import { InvalidError } from "@orchium/core/v1/config/error"
  ...
  if (permissions.length)
    throw new InvalidError({
      ...
      issues: permissions.map((path) => ({
        path,
        message: 'V2 permissions are not supported by Orchium V1. Use V1 "permission" rules or run opencode2.',
      })),
    })
