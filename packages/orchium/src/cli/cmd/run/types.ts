// Shared type vocabulary for the direct interactive mode (`orchium --mini`).
//
...
import type { OrchiumClient, PermissionRequest, QuestionRequest, ToolPart } from "@orchium/sdk/v2"
import type { TuiConfig } from "@orchium/tui/config"
...
type PromptModel = Parameters<OrchiumClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<OrchiumClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<OrchiumClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<OrchiumClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<OrchiumClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<OrchiumClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<OrchiumClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<OrchiumClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<OrchiumClient["question"]["reject"]>[0]
