import type { PermissionV1 } from "@orchium/core/v1/permission"
import { FSUtil } from "@orchium/core/fs-util"
// CLI entry point for `orchium run` and `orchium --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`orchium --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`orchium --mini --attach`): connects to a running
//      orchium server and runs interactive mode against it.
//
...
import { createOrchiumClient, type OrchiumClient, type ToolPart } from "@orchium/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<OrchiumClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run orchium with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running orchium server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to ORCHIUM_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to ORCHIUM_SERVER_USERNAME or 'orchium')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: OrchiumClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: OrchiumClient, sessionID: string) {
      ...
      async function current(sdk: OrchiumClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: OrchiumClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: OrchiumClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: OrchiumClient) {
        ...
        async function loop(client: OrchiumClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createOrchiumClient({
        baseUrl: "http://orchium.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
