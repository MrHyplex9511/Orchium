...
import type { AssistantMessage, Message, OrchiumClient, Session, SessionMessageResponse } from "@orchium/sdk/v2"
...
export const AuthMethodID = "orchium-login"
...
export class Service extends Context.Service<Service, Interface>()("@orchium/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `orchium auth login` in the terminal",
      name: "Login with orchium",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: OrchiumClient) {
...
function makeUsageService(sdk: OrchiumClient) {
...
async function loadDirectorySnapshot(sdk: OrchiumClient, directory: string) {
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Orchium service failure", service })
}
