...
import type { Event, OrchiumClient } from "@orchium/sdk/v2"
...
type PermissionReplyParams = Parameters<OrchiumClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "orchium-acp-permission-"))
