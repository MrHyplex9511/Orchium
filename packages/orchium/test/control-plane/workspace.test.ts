...
import { EventSequenceTable } from "@orchium/core/event/sql"
...
import { WorkspaceV2 } from "@orchium/core/workspace"
import { WorkspaceTable } from "@orchium/core/control-plane/workspace.sql"
...
const originalEnv = {
  ORCHIUM_AUTH_CONTENT: process.env.ORCHIUM_AUTH_CONTENT,
  ORCHIUM_EXPERIMENTAL_WORKSPACES: process.env.ORCHIUM_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.ORCHIUM_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@orchium.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.ORCHIUM_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=orchium-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.ORCHIUM_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.ORCHIUM_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.ORCHIUM_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=orchium-test")
