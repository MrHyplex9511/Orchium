...
import { ModelsDev } from "@orchium/core/models-dev"
import { HttpRecorder } from "@orchium/http-recorder"
import { HttpRecorderInternal } from "@orchium/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@orchium/llm"
import { RequestExecutor } from "@orchium/llm/route"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.ORCHIUM_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["orchium", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("ORCHIUM_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("ORCHIUM_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["orchium", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["orchium", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.ORCHIUM_RECORD_CONSOLE_TOKEN && process.env.ORCHIUM_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.opencode,
        name: "Orchium Zen",
        env: ["ORCHIUM_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.ORCHIUM_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.ORCHIUM_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.ORCHIUM_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["orchium", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("ORCHIUM_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("ORCHIUM_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("ORCHIUM_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set ORCHIUM_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the orchium LLM stack remain real.
  const metadata = {
