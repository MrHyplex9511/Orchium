...
import { LLMEvent, ToolFailure } from "@orchium/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@orchium/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("prefers console provider api key over stored orchium auth", () => {
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("opencode") },
        provider: {
