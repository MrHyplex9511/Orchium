...
import { LLMClient, RequestExecutor } from "@orchium/llm/route"
...
describe("session.llm.stream", () => {
  ...
  const orchiumFixture = { providerID: "orchium-test", modelID: vivgridFixture.modelID }

  it.instance(
    "sends the parent session header for orchium providers",
    () =>
      Effect.gen(function* () {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderV2.ID.make(orchiumFixture.providerID),
          ModelV2.ID.make(orchiumFixture.modelID),
        )
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(orchiumFixture.providerID),
            modelID: resolved.id,
          },
      ...
      }),
    {
      config: () => {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        return {
          enabled_providers: [orchiumFixture.providerID],
          provider: {
            [orchiumFixture.providerID]: {
              name: "Orchium Test",
