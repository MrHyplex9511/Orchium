...
it.instance("plan agent denies edits except .orchium/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".orchium/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".orchium", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.ORCHIUM_TEST_HOME
      process.env.ORCHIUM_TEST_HOME = test.directory
