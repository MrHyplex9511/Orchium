...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ orchium }) =>
    Effect.gen(function* () {
      const result = yield* orchium.spawn(["--replay-limit", "10"])

      orchium.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ orchium }) =>
    Effect.gen(function* () {
      const result = yield* orchium.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ orchium }) =>
    Effect.gen(function* () {
      const result = yield* orchium.spawn(["--mini", "--port", "4096"])
