...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/orchium.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/orchium.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/orchium.jsonc", dir: ".orchium", suggestion: "orchium" },
        expected:
          'Directory ".orchium" in /tmp/orchium.jsonc is not valid. Rename the directory to "orchium" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/orchium.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/orchium.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `orchium models` to list available models",
      "Or check your config (orchium.json) provider/model names",
    ].join("\n")
