...
describe("orchium read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // ORCHIUM_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ orchium }) =>
      Effect.gen(function* () {
        const r = yield* orchium.spawn(["mcp", "list"])
        orchium.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ orchium }) =>
      Effect.gen(function* () {
        const r = yield* orchium.spawn(["providers", "list"])
        orchium.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ orchium }) =>
      Effect.gen(function* () {
        const r = yield* orchium.spawn(["models"])
        orchium.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ orchium }) =>
      Effect.gen(function* () {
        const r = yield* orchium.spawn(["agent", "list"])
        orchium.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh ORCHIUM_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ orchium }) =>
      Effect.gen(function* () {
        const r = yield* orchium.spawn(["session", "list"])
        orchium.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ orchium }) =>
      Effect.gen(function* () {
        const r = yield* orchium.spawn(["stats"])
        orchium.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under ORCHIUM_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ orchium }) =>
      Effect.gen(function* () {
        const r = yield* orchium.spawn(["db", "path"])
        orchium.expectExit(r, 0, "db path")
