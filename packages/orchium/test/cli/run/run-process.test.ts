// Subprocess integration tests for `orchium run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `orchium.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `ORCHIUM_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("orchium run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, orchium }) =>
      Effect.gen(function* () {
        ...
        const result = yield* orchium.run("say hi")
        orchium.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, orchium }) =>
      Effect.gen(function* () {
        ...
        const result = yield* orchium.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, orchium }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* orchium.run("think", { extraArgs: ["--thinking"] })
        orchium.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* orchium.run("think again")
        orchium.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, orchium }) =>
      Effect.gen(function* () {
        ...
        const result = yield* orchium.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, orchium }) =>
      Effect.gen(function* () {
        ...
        const result = yield* orchium.run("say hi", { format: "json" })
        orchium.expectExit(result, 0)

        const events = orchium.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ orchium }) =>
      Effect.gen(function* () {
        const result = yield* orchium.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, orchium }) =>
      Effect.gen(function* () {
        ...
        const result = yield* orchium.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, orchium }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* orchium.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, orchium }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* orchium.run("request permission", { permission: { bash: "ask" } })
        orchium.expectExit(denied, 0)
        ...
        const allowed = yield* orchium.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        orchium.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* orchium.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        orchium.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, orchium }) =>
      Effect.gen(function* () {
        ...
        const server = yield* orchium.serve()

        const result = yield* orchium.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, orchium }) =>
      Effect.gen(function* () {
        const result = yield* orchium.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, orchium }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* orchium.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
