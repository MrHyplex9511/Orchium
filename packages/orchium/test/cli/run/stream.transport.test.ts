...
import { OrchiumClient, type GlobalEvent } from "@orchium/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<OrchiumClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<OrchiumClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<OrchiumClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<OrchiumClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<OrchiumClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://orchium.test"),
...
function sdk(
  input: {
    ...
    subscribe?: OrchiumClient["event"]["subscribe"]
    globalEvent?: OrchiumClient["global"]["event"]
    promptAsync?: OrchiumClient["session"]["promptAsync"]
    status?: OrchiumClient["session"]["status"]
    messages?: OrchiumClient["session"]["messages"]
    children?: OrchiumClient["session"]["children"]
    permissions?: OrchiumClient["permission"]["list"]
    questions?: OrchiumClient["question"]["list"]
  } = {},
) {
  const client = new OrchiumClient()

  const subscribe: OrchiumClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: OrchiumClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: OrchiumClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: OrchiumClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: OrchiumClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: OrchiumClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: OrchiumClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: OrchiumClient["question"]["list"] = input.questions ?? (() => ok([]))
