...
describe("orchium acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ orchium }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ orchium }))
        ...
        expect(initialized.agentInfo?.name).toBe("Orchium")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ orchium }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ orchium })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("orchium-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "orchium-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.ORCHIUM_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ orchium }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("orchium-login")
