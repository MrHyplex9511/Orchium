...
export function createAcpClient(input: Pick<CliFixture, "orchium">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.orchium.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "orchium-local-acp", version: "0.1.0" },
      }),
    )
  })
}
