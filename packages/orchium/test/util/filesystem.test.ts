...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "orchium.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "orchium.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "orchium.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "orchium.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["orchium.json", "orchium.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "orchium.json"),
        path.join(project, "orchium.jsonc"),
      ])
    })
  })
