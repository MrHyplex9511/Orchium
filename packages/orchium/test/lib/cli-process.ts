// Subprocess test harness for the orchium CLI. Spawns the real binary against
...
//
// Configuration flows through orchium's built-in test affordances:
//   - ORCHIUM_CONFIG_CONTENT      : provider config inline, no files to find
//   - ORCHIUM_TEST_HOME           : pins os.homedir() → tmpdir
//   - ORCHIUM_DISABLE_PROJECT_CONFIG : skip walking up for orchium.json
//   - ORCHIUM_PURE                : skip external plugin discovery + install
//   - ORCHIUM_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `orchium.run` is fully wired. The shape supports adding more
// builders (`orchium.serve(opts)`, `orchium.acp(opts)`, `orchium.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OrchiumCli.
...
import { AppProcess } from "@orchium/core/process"
...
const orchiumRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(orchiumRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ORCHIUM_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    ORCHIUM_CONFIG_CONTENT: configJson,
    ORCHIUM_DISABLE_PROJECT_CONFIG: "1",
    ORCHIUM_PURE: "1",
    ORCHIUM_DISABLE_AUTOUPDATE: "1",
    ORCHIUM_DISABLE_AUTOCOMPACT: "1",
    ORCHIUM_DISABLE_MODELS_FETCH: "1",
    ORCHIUM_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `orchium run`. New flags should
...
// `orchium serve` is a long-lived process — it never exits on its own.
...
// `orchium acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OrchiumCli = {
  ...
  // Spawn `orchium serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `orchium acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("orchium.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          ORCHIUM_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("orchium.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("orchium.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "orchium server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `orchium serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("orchium.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const orchium: OrchiumCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, orchium })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `orchium.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "orchium") {
...
// every call site. Use this for any test that needs the orchium CLI fixture.
//
...
// resources (e.g. `orchium.serve`) without an extra `Effect.scoped` wrapper —
