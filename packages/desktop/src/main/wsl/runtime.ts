...
export async function installWslOrchium(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
...
export async function resolveWslOrchium(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        'if [ -x "$HOME/.orchium/bin/orchium" ]; then printf "%s\\n" "$HOME/.orchium/bin/orchium"; fi',
        distro,
        opts,
      )
    ).stdout,
