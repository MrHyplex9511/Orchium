...
import { type WslCommandLine, resolveWslOrchium, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const orchium = await resolveWslOrchium(distro)
  if (!orchium) throw new Error(nativeT("desktop.wsl.error.orchiumNotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = "orchium"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export ORCHIUM_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export ORCHIUM_CLIENT=desktop",
    `export ORCHIUM_SERVER_USERNAME=${shellEscape(username)}`,
    `export ORCHIUM_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(orchium)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
