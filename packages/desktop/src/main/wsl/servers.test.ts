...
let releaseOrchiumResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectOrchiumVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectOrchiumVersion("1.14.35", "1.16.2")).toThrow(
    "Orchium update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Orchium", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.orchium/bin/orchium",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, orchiumChecks: {} })
})
...
test("ignores stale background Orchium checks after removing a WSL server", async () => {
  persistedServers = []
  releaseOrchiumResolve = undefined
  ...
  await waitFor(() => !!releaseOrchiumResolve)
  await controller.removeServer("wsl:Debian")
  releaseOrchiumResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().orchiumChecks).toEqual({})
})

test("ignores stale startup Orchium checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking Orchium", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const orchium: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveOrchium: async (distro) => {
      orchium.push(distro)
      return "/home/me/.orchium/bin/orchium"
    },
  })
  ...
  expect(orchium).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(orchium).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().orchiumChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check Orchium in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const orchium: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(orchium).toEqual(["Debian"])
  expect(Object.keys(controller.getState().orchiumChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolveOrchium: async () => {
      await new Promise<void>((resolve) => {
        releaseOrchiumResolve = resolve
      })
      return "/home/me/.orchium/bin/orchium"
    },
  }
