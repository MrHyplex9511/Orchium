...
import { expectOrchiumVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveOrchium?: typeof resolveWslOrchium
...
export function createWslServersController(
  ...
  const setOrchiumCheck = (distro: string, check: WslOrchiumCheck) => {
    setState({
      orchiumChecks: {
        ...state.orchiumChecks,
        [distro]: check,
  ...
  const checkOrchium = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveOrchium ?? resolveWslOrchium)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return orchiumCheck(distro, resolved, version, appVersion)
  }

  const refreshOrchiumCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setOrchiumCheck(distro, await checkOrchium(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const orchiumChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.orchiumChecks[distro])
        .map(async (distro) => [distro, await checkOrchium(distro, opts)] as const),
    )
    if (orchiumChecks.length) {
      setState({ orchiumChecks: { ...state.orchiumChecks, ...Object.fromEntries(orchiumChecks) } })
    }
  }
  ...
  const refreshOrchiumCheckBackground = (id: string, distro: string) => {
    void checkOrchium(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setOrchiumCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl orchium check failed", { id, distro, message })
      })
  }

  const refreshOrchiumChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkOrchium(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setOrchiumCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl orchium check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshOrchiumCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installOrchium(name: string) {
      await runJob({ kind: "install-orchium", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslOrchium(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || nativeT("desktop.wsl.error.installOrchium"))
        }
        await refreshOrchiumCheck(name, { signal: abort.signal })
        expectOrchiumVersion(state.orchiumChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.orchiumChecks, distro) : {}),
      })
    },
...
function orchiumCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.orchiumMissing"),
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.orchiumCannotRun"),
    }
  }
  return {
    distro,
