...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier Orchium launch", () => {
    expect(hasExistingAppState([file("orchium.settings")])).toBe(true)
    expect(hasExistingAppState([file("orchium.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("orchium")])).toBe(true)
  })
})
