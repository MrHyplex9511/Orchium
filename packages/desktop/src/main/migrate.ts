...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.orchium.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `orchium.settings.dat` is special: it maps to the `orchium.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"orchium.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // orchium.settings.dat → the electron settings store ("orchium.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "orchium.global.dat").
  const storeName = filename === "orchium.settings.dat" ? "orchium.settings" : filename
