...
import type { UpdaterState } from "@orchium/app/updater"
...
import "./styles.css"
import { Splash } from "@orchium/ui/logo"
import { useTheme } from "@orchium/ui/theme/context"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.ORCHIUM_CHANNEL === "prod" &&
...
const deepLinkEvent = "orchium:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__ORCHIUM__ ??= {}
  const pending = window.__ORCHIUM__.deepLinks ?? []
  window.__ORCHIUM__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `orchium.desktop.window.${windowID}.last-active-url`
}
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("orchium.global.dat").getItem("language")
