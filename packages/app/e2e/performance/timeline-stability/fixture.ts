...
import { SessionStatusEvent } from "@orchium/schema/session-status-event"
import { SessionV1 } from "@orchium/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/Orchium/TimelineStability"
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("orchium.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
