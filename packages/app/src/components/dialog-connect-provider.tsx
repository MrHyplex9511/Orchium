import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@orchium/client/promise"
import { Button } from "@orchium/ui/button"
import { useDialog } from "@orchium/ui/context/dialog"
import { Dialog } from "@orchium/ui/dialog"
import { Icon } from "@orchium/ui/icon"
import { IconButton } from "@orchium/ui/icon-button"
import { List, type ListRef } from "@orchium/ui/list"
import { ProviderIcon } from "@orchium/ui/provider-icon"
import { Spinner } from "@orchium/ui/spinner"
import { Tag } from "@orchium/ui/tag"
import { TextField } from "@orchium/ui/text-field"
import { ButtonV2 } from "@orchium/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@orchium/ui/v2/dialog-v2"
import { TextInputV2 } from "@orchium/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "opencode-go") return language.t("dialog.provider.orchiumGo.tagline")
    return undefined
  }
      ...
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <Show when={i.id === "opencode"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.orchium.tagline")}</div>
          </Show>
                          ...
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "opencode"
                                ? "dialog.provider.orchium.tagline"
                                : "dialog.provider.orchiumGo.tagline",
                            )}
                          </span>
        ...
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          ...
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.orchiumZen.line1")}</div>
              <div>{language.t("provider.connect.orchiumZen.line2")}</div>
              <div>
                {language.t("provider.connect.orchiumZen.visit.prefix")}
                <ExternalLink
                  href="https://opencode.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.orchiumZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.orchiumZen.visit.suffix")}
              </div>
            </div>
          </Show>
        ...
        <Switch>
          <Match when={provider().id === "opencode"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.orchiumZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.orchiumZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.orchiumZen.visit.prefix")}
