...
import { NormalizedProviderListResponse } from "@orchium/session-ui/context"
...
export const loadLspQuery = (scope: ServerScope, directory: string, sdk: OrchiumClient) =>
  queryOptions({
...
function makeQueryOptionsApi(
  scope: ServerScope,
  serverSDK: () => OrchiumClient,
  serverAPI: ServerApi,
  sdkFor: (dir: PathKey) => OrchiumClient,
  protocol: Promise<"v1" | "v2">,
) {
  return {
...
export function createServerSyncContextInner(serverSDK: ServerSDK) {
  ...
  const sdkCache = new Map<string, OrchiumClient>()
