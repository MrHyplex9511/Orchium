  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://orchium.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://orchium.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://orchium.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://orchium.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://orchium.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://orchium.example.com"),
      ),
    ).toEqual({
