...
import { Tag } from "@orchium/ui/v2/badge-v2"
...
import { wslOrchiumAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.orchiumChecks[item.config.distro]
          const orchiumAction = () => wslOrchiumAction(check())
          const busy = () => wsl.data?.job?.kind === "install-orchium" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={orchiumAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installOrchium(item.config.distro))}
                    >
