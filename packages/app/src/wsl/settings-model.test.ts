...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Orchium needs attention", () => {
    expect(wslOrchiumAction(undefined)).toBeUndefined()
    expect(
      wslOrchiumAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("wsl.onboarding.installOrchium")
    expect(
      wslOrchiumAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/orchium",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("wsl.onboarding.updateOrchium")
    expect(
      wslOrchiumAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/orchium",
  ...
  test("does not report ready when Orchium is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        orchiumChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.orchium/bin/orchium",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "orchium is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installOrchium" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-orchium")
  })
