...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-orchium" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslOrchiumAction(check?: WslOrchiumCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installOrchium"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateOrchium"
}
...
export function addServerViewModel(input: {
  ...
  const orchiumCheck = selectedDistro ? (state?.orchiumChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.orchiumChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateOrchium" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.orchiumMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installOrchium" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  orchiumCheck: WslOrchiumCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingOrchium =
    input.state?.job?.kind === "install-orchium" && input.state.job.distro === input.selectedDistro
  if (!ready || probingOrchium) {
    return {
      variant: "contrast",
  ...
  if (!addServerOrchiumReady(input.orchiumCheck)) {
    const update = !!input.orchiumCheck?.resolvedPath && input.orchiumCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingOrchium
        ? { key: "wsl.onboarding.updatingOrchium" }
        : update
          ? { key: "wsl.onboarding.updateOrchium" }
          : { key: "wsl.onboarding.installOrchium" },
      disabled: !!input.state?.job || input.adding,
      action: "install-orchium",
      loading: installingOrchium,
...
function addServerOrchiumReady(check: WslOrchiumCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.orchiumChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.orchiumChecks[item.name]) return [`orchium:${item.name}`]
    return []
  })
