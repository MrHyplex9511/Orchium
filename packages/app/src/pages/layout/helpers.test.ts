...
import { type Session } from "@orchium/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  compareSessionTime,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("orchium://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("orchium://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("orchium://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("orchium://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("orchium://open-project")).toBeUndefined()
    expect(parseDeepLink("orchium://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "orchium://open-project?directory=/a",
      "orchium://other?directory=/b",
      "orchium://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("orchium://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("orchium://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("orchium://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("orchium://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "orchium://new-session?directory=/a",
      "orchium://open-project?directory=/b",
      "orchium://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __ORCHIUM__: {
        deepLinks: ["orchium://open-project?directory=/a"],
      },
    } as unknown as Window & { __ORCHIUM__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["orchium://open-project?directory=/a"])
