...
const DEFAULT_SERVER_URL_KEY = "orchium.settings.dat:defaultServerUrl"
...
const getCurrentUrl = () => {
  ...
  if (import.meta.env.DEV)
    return `http://${import.meta.env.VITE_ORCHIUM_SERVER_HOST ?? "localhost"}:${import.meta.env.VITE_ORCHIUM_SERVER_PORT ?? "4096"}`
  ...
  Sentry.init({
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" && !(import.meta.env.ORCHIUM_CHANNEL === "prod" && i.name === "GlobalHandlers"),
      )
    },
  })
}
