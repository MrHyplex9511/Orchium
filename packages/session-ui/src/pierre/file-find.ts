...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("orchium-find")
  api.delete("orchium-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("orchium-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("orchium-find", new Highlight(...rest))
    return true
  }
