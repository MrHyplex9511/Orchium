...
This is the living design and status document for `@orchium/codemode` and its existing V2 Orchium adapter.
...
`packages/orchium/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
