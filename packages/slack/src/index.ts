...
import { createOrchium, type ToolPart } from "@orchium/sdk"
...
console.log("🚀 Starting orchium server...")
const orchium = await createOrchium({
  port: 0,
})
console.log("✅ Orchium server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await orchium.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new orchium session...")
    const { client, server } = orchium
    ...
    console.log("✅ Created orchium session:", createResult.data.id)
  ...
  console.log("📝 Sending to orchium:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Orchium response:", JSON.stringify(result, null, 2))
