...
import { getOrchiumModeStack, ORCHIUM_BASE_MODE, OrchiumKeymapProvider, registerOrchiumKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerOrchiumKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when orchium mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getOrchiumModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getOrchiumModeStack(keymap).push("autocomplete")
