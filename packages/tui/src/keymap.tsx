...
export const ORCHIUM_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const ORCHIUM_MODE_KEY = "orchium.mode"

export const OrchiumKeymapProvider = KeymapProvider
export const useOrchiumKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type OrchiumModeStack = ReturnType<typeof createOrchiumModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, OrchiumModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createOrchiumModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(ORCHIUM_MODE_KEY, ORCHIUM_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(ORCHIUM_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(ORCHIUM_MODE_KEY, stack.at(-1)?.mode ?? ORCHIUM_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? ORCHIUM_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(ORCHIUM_MODE_KEY, undefined)
...
export function useOrchiumModeStack() {
  return getOrchiumModeStack(useOrchiumKeymap())
}

export function getOrchiumModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Orchium mode stack is not registered for this keymap")
  return value
}
...
export function registerOrchiumKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createOrchiumModeStack(keymap)
