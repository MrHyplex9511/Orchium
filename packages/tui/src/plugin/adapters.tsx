import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@orchium/plugin/tui"
...
import type { useOrchiumKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useOrchiumKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getOrchiumModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getOrchiumModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
