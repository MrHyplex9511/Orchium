...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "orchium/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".orchium/glossary/zh-cn.md"
  if (locale === "zht") return ".orchium/glossary/zh-tw.md"
  return `.orchium/glossary/${locale}.md`
}
...
async function translate(
  ...
  const exported = Bun.spawn(["orchium", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.OPENCODE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["orchium", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
