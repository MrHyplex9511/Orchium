...
import { createOrchium } from "@orchium-ai/sdk"
...
`)
  ...
  const orchium = await createOrchium({ port: 0 })
  ...
  try {
    ...
    const session = await orchium.client.session.create()
    const result = await orchium.client.session
      .prompt({
